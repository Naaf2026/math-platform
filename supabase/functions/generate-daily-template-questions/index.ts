...
function evalRule(node:any,vars:any,answers:any={}):any{
  ...
  const op=node.op,args=Array.isArray(node.args)?node.args.map((x:any)=>Number(evalRule(x,vars,answers))):[];
...
function visual(type:string,cfg:any,vars:any,answers:any){
 ...
 if(type==="number_line"){const start=Number(vars.start??vars.starting_number??0),forward=Number(vars.forward??vars.steps_forward??0),backward=Number(vars.backward??vars.steps_backward??0);return {...v,type:"number_line",start,forward,backward,firstResult:answers.alima??answers.first??start+forward,secondResult:answers.ibrahim??answers.second??start+forward-backward};}
...
Deno.serve(async(req)=>{if(req.method==="OPTIONS")return new Response("ok",{headers:cors});try{
 ...
 for(const t of templates??[]){try{
   ...
   const prompt=render(t.template_text,vars);const visualCfg=visual(t.question_type,t.illustration_config,vars,answers);const interaction={...(t.interaction_config||{})};if(["number_line","visual_table","visual_question"].includes(t.question_type))interaction.visual=visualCfg;
   const answer=Object.keys(answers).length===1?answers[Object.keys(answers)[0]]:answers;const topicId=(t.template_name.toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,"")||"textbook");
   ...
   const existing=await db.from("question_template_instances").select("id,learning_question_id").eq("template_id",t.id).eq("generated_on",day).limit(1);if(existing.data?.length){results.push({template_id:t.id,status:"already_generated"});continue;}
   const qid=`daily-tpl-${day.replace(/[^0-9]/g,"")}-${t.id.replace(/-/g,"").slice(0,10)}`;
   const {error:qe}=await db.from("learning_questions").insert({id:qid,topic_id:topicId,prompt,options:[],answer:typeof answer==="object"?JSON.stringify(answer):String(answer),explanation:"Solve the question using the same method shown in the textbook exercise.",sort_order:seed,difficulty:"medium",skill:"textbook-template",question_type:t.question_type,interaction_config:interaction,hint:"",animation:"question-enter",media_url:null,grade:Number.isFinite(grade)?grade:null,source_book_id:t.book_id,source_chapter_id:t.chapter_id,source_page_start:t.source_page_start,source_page_end:t.source_page_end,template_id:t.id});
   if(qe){results.push({template_id:t.id,status:"insert_failed",error:qe.message});continue;}
   const {error:ie}=await db.from("question_template_instances").insert({template_id:t.id,learning_question_id:qid,values:vars,generated_on:day});if(ie){results.push({template_id:t.id,status:"instance_failed",error:ie.message});continue;}
