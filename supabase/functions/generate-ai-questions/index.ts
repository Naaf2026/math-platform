...
const responseSchema = {
  type: 'object', properties: { questions: { type: 'array', items: { type: 'object', properties: {
    ...
    correct_answer: { type: 'object', properties: { value: { type: 'string' } }, required: ['value'] }, explanation: { type: 'string' }, hint: { type: 'string' }, interaction_config: { type: 'object' }, source_reference: { type: 'object' }, ai_confidence: { type: 'number' }
  }, required: ['prompt','question_type','difficulty','skill','topic','options','correct_answer','explanation','hint','interaction_config','source_reference','ai_confidence'] } } }, required: ['questions']
};
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: job, error: jobError } = await admin.from('ai_generation_jobs').insert({ requested_by: user.id, book_id: book.id, chapter_id: chapter?.id ?? null, learning_objective_id: objective?.id ?? null, grade, age, subject: book.subject, difficulty, question_types, requested_count: count, status: 'processing', model }).select('id').single();
    ...
    try {
      ...
      if (!validRows.length) throw new Error(`Gemini generated no valid questions (${generated.length} returned)`);
      ...
      return json({ job_id: job.id, generated_count: validRows.length, status: 'completed', model });
    } catch (error) {
      const detail = describeError(error);
      await admin.from('ai_generation_jobs').update({ status: 'failed', error_message: detail.slice(0, 2000) }).eq('id', job.id);
      return json({ error: 'Question generation failed', job_id: job.id, detail, model }, 200);
    }
  } catch (error) {
