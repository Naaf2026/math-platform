...
async function uploadToGemini(bytes: ArrayBuffer, displayName: string, apiKey: string) {
  ...
  if (!file?.uri) throw new Error('Gemini PDF upload returned no file URI');
...
async function waitForGeminiFile(name: string, apiKey: string) {
  for (let i = 0; i < 12; i++) {
    ...
    if (response.ok) {
      ...
      if (file.state === 'FAILED') throw new Error('Gemini failed to process the PDF');
    }
...
Deno.serve(async (req) => {
  ...
  const { data: isAdmin, error: roleError } = await admin.rpc('has_role', { p_role: 'admin' });
...
`;
      ...
      if (!response.ok) {
        ...
        throw new Error(`Gemini indexing failed (${response.status}): ${detail.slice(0, 500)}`);
      }

      const result = await response.json();
      const text = textFromParts(result);
      if (!text) throw new Error('Gemini returned no indexing result');
      ...
      await admin.from('learning_objectives').delete().eq('book_id', book.id);
      await admin.from('book_chapters').delete().eq('book_id', book.id);
      ...
      await admin.from('books').update({ processing_status: 'failed', processing_error: message.slice(0, 2000) }).eq('id', book.id);
      return json({ error: 'Book indexing failed', detail: message }, 502);
    }
  } catch (error) {
    console.error(error);
