...
async function waitForGeminiFile(name: string, apiKey: string) {
  for (let i = 0; i < 12; i++) {
    ...
    if (response.ok) {
      ...
      throw new Error(`Gemini file status check failed (${response.status}): ${(await response.text()).slice(0, 500)}`);
    }
...
Deno.serve(async (req) => {
  ...
  if (!supabaseUrl || !serviceRoleKey || !geminiKey) {
    console.error('[process-book] missing required environment configuration', {
  ...
  try {
    ...
    try {
      ...
      console.log('[process-book] PDF downloaded', { bookId: book.id, bytes: bytes.byteLength });
      if (bytes.byteLength === 0) throw new Error('Stored PDF is empty');
      ...
      console.log('[process-book] Gemini file uploaded', { bookId: book.id, name: geminiFile.name });
      ...
      const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent?key=${encodeURIComponent(geminiKey)}`, {
        ...
        body: JSON.stringify({
          contents: [{ role: 'user', parts: [{ text: prompt }, { file_data: { mime_type: geminiFile.mimeType, file_uri: geminiFile.uri } }] }],
          generationConfig: { responseMimeType: 'application/json', responseJsonSchema: extractionSchema, temperature: 0.1 },
        }),
      });
      if (!response.ok) throw new Error(`Gemini indexing failed (${response.status}): ${(await response.text()).slice(0, 1000)}`);
      const result = await response.json();
      const text = textFromParts(result);
      ...
      console.log('[process-book] Gemini extraction complete', { bookId: book.id, chapters: chapters.length, objectives: objectives.length });
      ...
      console.error('[process-book] FAILED', { bookId: book.id, stage, error: message });
      await admin.from('books').update({ processing_status: 'failed', processing_error: `${stage}: ${message}`.slice(0, 2000) }).eq('id', book.id);
      return json({ error: 'Book indexing failed', stage, detail: message }, 502);
    }
  } catch (error) {
    const message = describeError(error);
    console.error('[process-book] UNEXPECTED', { stage, error: message });
