 ...
 try{
  ...
  for(const q of questions){
   try{
    ...
    const prompt=`You are a strict curriculum question validator. Validate the supplied mathematics question using ONLY the supplied curriculum source. Do not rewrite it. Check mathematical correctness, answer validity, grade/age appropriateness, curriculum alignment, difficulty, and duplicates. Return a conservative quality score 0-100. A question is approved only when it is mathematically correct, has a valid unambiguous answer, is appropriate for the stated learner, and is supported by the source. Context: ${JSON.stringify(context)}`;
    ...
    const pass=Boolean(v.mathematically_correct&&v.answer_valid&&v.grade_appropriate&&v.age_appropriate&&v.curriculum_aligned&&v.difficulty_valid&&!v.duplicate_detected&&score>=80);
    const status=pass?'approved':score<55||!v.mathematically_correct||!v.answer_valid?'rejected':'needs_review';
    ...
    const {error:updateError}=await db.from('ai_generated_questions').update({validation_status:status,validation_errors:status==='approved'?[]:[String(v.validator_notes||'Validation requires review')]}).eq('id',q.id); if(updateError)throw new Error(`Question status update failed: ${describe(updateError)}`);
    results.push({id:q.id,status,quality_score:score,mathematically_correct:!!v.mathematically_correct,answer_valid:!!v.answer_valid,grade_appropriate:!!v.grade_appropriate,age_appropriate:!!v.age_appropriate,curriculum_aligned:!!v.curriculum_aligned,difficulty_valid:!!v.difficulty_valid,duplicate_detected:!!v.duplicate_detected,validator_notes:String(v.validator_notes||'')});
