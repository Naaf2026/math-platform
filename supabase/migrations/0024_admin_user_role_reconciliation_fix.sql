...
-- Migration 0024
-- Fix for existing admin user directory return type
-- ============================================================
-- PostgreSQL does not allow CREATE OR REPLACE FUNCTION to change
-- the row type defined by OUT/RETURNS TABLE parameters.
-- Drop the previous function before recreating it with the
-- unassigned-role column behavior introduced in migration 0023.
...
revoke all
on function public.get_admin_user_directory(text)
from public, anon;
...
-- Recreate the dashboard RPC with the unassigned count.
...
revoke all
on function public.get_admin_dashboard_analytics()
from public, anon;
