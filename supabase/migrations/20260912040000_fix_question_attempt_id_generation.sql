-- Fix Daily Mission answer submission when question_attempts.id has no default.
-- Explicitly generate the attempt UUID so the critical answer insert succeeds.
...
begin
  ...
  insert into public.question_attempts(id,user_id,question_id,selected_answer,is_correct,xp_awarded)
  values(v_attempt_id,v_user_id,p_question_id,p_selected_answer,v_correct,0);
  ...
  begin
    ...
    update public.topic_progress set mastery=least(100,round((v_correct_count::numeric/greatest(v_answered,1))*100)::integer)
      where user_id=v_user_id and topic_id=v_topic_id;
  exception when others then null;
  end;

  begin perform public.record_learning_activity(); exception when others then null; end;
