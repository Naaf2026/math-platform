...
create table if not exists public.parent_learning_alerts (
  ...
  related_goal_id uuid null references public.parent_learning_goals(goal_id) on delete set null,
...
drop policy if exists parent_learning_alerts_parent_select
...
create or replace function public.mark_parent_learning_alert_read(
  p_alert_id uuid
)
returns boolean
language plpgsql
...
create or replace function public.generate_parent_learning_alerts(
  p_student_id uuid
)
returns integer
language plpgsql
...
begin
  ...
  select
    count(distinct qa.created_at::date)::integer,
    count(*)::integer
  ...
  if v_recent_questions = 0 then
    ...
    values (
      p_student_id,
      'no_recent_practice',
      'Time for a learning session',
      'There has been no recorded practice in the last 7 days. A short session can help keep learning momentum going.',
      1
    ...
    values (
      ...
      'Build a steadier learning habit',
      'The learner has practiced on ' || v_recent_days || ' of the last 7 days. A few short, consistent sessions could strengthen progress.',
      2
    ) on conflict (student_id, alert_type, title) do nothing;
    if found then v_created := v_created + 1; end if;
  end if;
  ...
  if v_accuracy > 0 and v_accuracy < 60 then
    ...
    values (
      p_student_id,
      'accuracy_attention',
      'Accuracy needs some extra support',
      'Overall accuracy is currently ' || round(v_accuracy)::text || '%. Focused review and practice may help strengthen understanding.',
      1
    ) on conflict (student_id, alert_type, title) do nothing;
    if found then v_created := v_created + 1; end if;
    ...
    values (
      ...
      'Overall accuracy is currently ' || round(v_accuracy)::text || '%. Keep encouraging consistent practice.',
      3
    ) on conflict (student_id, alert_type, title) do nothing;
    if found then v_created := v_created + 1; end if;
  end if;
  ...
  if v_streak >= 7 then
    ...
    values (
      ...
      'The learner has reached a ' || v_streak::text || '-day learning streak. Celebrate the consistency!',
      3
    ) on conflict (student_id, alert_type, title) do nothing;
    if found then v_created := v_created + 1; end if;
  end if;
  ...
  for v_goal in
    select goal_id, title, due_date, progress_percent
    from public.parent_learning_goals
  ...
  loop
    ...
    values (
      ...
      'The goal "' || v_goal.title || '" is due on ' || to_char(v_goal.due_date,'Mon DD') || '. Current progress is ' || round(coalesce(v_goal.progress_percent,0))::text || '%.',
      1,
      v_goal.goal_id
    ) on conflict (student_id, alert_type, title) do nothing;
    if found then v_created := v_created + 1; end if;
