...
export default function ProgressPage(){
 ...
 const[loading,setLoading]=useState(true); const[filter,setFilter]=useState<"all"|"not-started"|"in-progress"|"completed">("all");
 ...
 <aside className="space-y-4"><section className="rounded-[22px] border border-[#dceaf8] bg-white p-5 shadow-sm"><div className="flex items-center justify-between"><h2 className="text-lg font-black">Recent Achievements</h2><Link href="/rewards" className="text-sm font-bold text-[#117ae9]">View All</Link></div><div className="mt-5 grid grid-cols-4 gap-2 text-center">{([{icon:"🎓",label:"First Lesson"},{icon:"⭐",label:"100 XP"},{icon:"🔥",label:"3-Day Streak"},{icon:"📖",label:"Topic Complete"}]).map(x=><div key={x.label}><span className="text-3xl">{x.icon}</span><p className="mt-2 text-[10px] font-bold">{x.label}</p></div>)}</div><p className="mt-3 text-xs text-[#7489a4]">Visit Rewards to see which badges you have earned.</p></section>
 ...
 <section className="rounded-[22px] border border-[#dceaf8] bg-white p-5 shadow-sm"><h2 className="text-lg font-black">Learning Journey</h2><div className="mt-4 flex items-center gap-3"><span className="text-5xl">📚</span><div><p className="text-sm text-[#7288a0]">Recorded activity</p><p className="text-2xl font-black">{missionsThisWeek} sessions</p><p className="text-xs text-[#7288a0]">This week · Time tracking coming soon</p></div></div></section></aside>
 </div></div></main>;
}
