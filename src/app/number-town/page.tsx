...
import NumberSchoolModal from "@/components/number-school-modal";
...
export default function NumberTownPage() {
  ...
  const exitFullscreen = useCallback(async () => { if (document.fullscreenElement && document.exitFullscreen) { try { await document.exitFullscreen(); } catch {} } }, []);
  const enterFullscreen = useCallback(async () => { if (document.fullscreenElement) return; try { if (document.documentElement.requestFullscreen) await document.documentElement.requestFullscreen(); } catch {} }, []);
  const startGame = useCallback(() => { setIndex(0); setScore(0); setMessage(""); setReward(null); setActiveLandmark(null); setSchoolOpen(false); setScreen("game"); void enterFullscreen(); }, [enterFullscreen]);
  const openSchool = useCallback(() => { setActiveLandmark(null); setSchoolOpen(true); }, []);
  const tryInteract = useCallback(() => { if (nearPortal) startGame(); else if (nearSchool) openSchool(); else if (nearShop) setActiveLandmark("shop"); else if (nearPark) setActiveLandmark("park"); }, [nearPortal, nearSchool, nearShop, nearPark, startGame, openSchool]);

  useEffect(() => {
    if (screen !== "world") return;
    const down = (event: KeyboardEvent) => { const key = event.key.toLowerCase(); if (["arrowup","arrowdown","arrowleft","arrowright","w","a","s","d","e","enter"].includes(key)) { event.preventDefault(); keysRef.current[key] = true; if ((key === "e" || key === "enter") && (nearPortal || nearSchool || nearShop || nearPark)) tryInteract(); } };
  ...
  useEffect(() => {
    ...
    const tick = (time: number) => { const previous = lastTimeRef.current ?? time; const dt = Math.min((time - previous) / 16.67, 2); lastTimeRef.current = time; let dx=0,dy=0; const k=keysRef.current; if(k.arrowleft||k.a)dx-=1;if(k.arrowright||k.d)dx+=1;if(k.arrowup||k.w)dy-=1;if(k.arrowdown||k.s)dy+=1;dx+=joystickRef.current.x;dy+=joystickRef.current.y;const length=Math.hypot(dx,dy);if(length>1){dx/=length;dy/=length;}setMoving(length>0.08);if(length>0.08)setPlayer(current=>{const proposed={x:Math.max(2,Math.min(94,current.x+dx*SPEED*dt)),y:Math.max(9,Math.min(88,current.y+dy*SPEED*dt))};const r:Rect={x:proposed.x-PLAYER.w/2,y:proposed.y-PLAYER.h/2,w:PLAYER.w,h:PLAYER.h};return OBSTACLES.some(o=>intersects(r,o))?current:proposed;});frameRef.current=window.requestAnimationFrame(tick);};
  ...
  useEffect(()=>{setNearPortal(distance(player,{x:50,y:46})<12);setNearSchool(distance(player,{x:18,y:34})<13);setNearShop(distance(player,{x:86,y:38})<13);setNearPark(distance(player,{x:50,y:67})<13);setNearLocked(distance(player,{x:86,y:80})<13);},[player]);
  useEffect(()=>{if(screen!=="game"&&document.fullscreenElement)void exitFullscreen();},[screen,exitFullscreen]);

  async function choose(value:number){if(message)return;const correct=value===q.target;const nextScore=score+(correct?1:0);setScore(nextScore);setMessage(correct?"✓ Great!":`Try again next time — the answer was ${q.target}.`);window.setTimeout(async()=>{if(index===QUESTIONS.length-1){setSaving(true);const{data,error}=await supabase.rpc("complete_number_town_game",{p_score:nextScore,p_total:QUESTIONS.length});if(!error&&data)setReward(data);setSaving(false);setScreen("result");}else{setIndex(v=>v+1);setMessage("");}},700);}
  const updateJoystick=(event:PointerEvent<HTMLDivElement>)=>{if(!joystickRef.current.active)return;const rect=event.currentTarget.getBoundingClientRect();const max=rect.width*0.32;const rawX=event.clientX-(rect.left+rect.width/2);const rawY=event.clientY-(rect.top+rect.height/2);const length=Math.min(max,Math.hypot(rawX,rawY));const angle=Math.atan2(rawY,rawX);const next={x:Math.cos(angle)*(length/max),y:Math.sin(angle)*(length/max)};joystickRef.current.x=next.x;joystickRef.current.y=next.y;setJoystick(next);};
  const joystickStart=(event:PointerEvent<HTMLDivElement>)=>{event.currentTarget.setPointerCapture(event.pointerId);joystickRef.current.active=true;updateJoystick(event);};const joystickEnd=()=>{joystickRef.current.active=false;joystickRef.current.x=0;joystickRef.current.y=0;setJoystick({x:0,y:0});};
  const interaction=nearPortal?{action:"PLAY GAME",icon:<Gamepad2 size={18}/>} : nearSchool?{action:"ENTER SCHOOL",icon:<Sparkles size={18}/>} : nearShop?{action:"OPEN SHOP",icon:<Coins size={18}/>} : nearPark?{action:"EXPLORE",icon:<Star size={18}/>} : null;
  ...
  return <main className="nt-shell"><div className="nt-top"><button className="icon-btn" aria-label="Back to Game Hub" onClick={()=>router.push("/games")}><ArrowLeft size={20}/></button><div><strong>Number Town</strong><small>World 1 · Level 1</small></div><button className="pill" onClick={()=>router.push("/games")}><Gamepad2 size={16}/> Game Hub</button></div><section className="nt-world"><div className="nt-sky"/><div className="nt-road r1"/><div className="nt-road r2"/><div className="nt-hill h1"/><div className="nt-hill h2"/>
    <button className="landmark school landmark-button" onClick={openSchool} aria-label="Open Number School">🏫<span>Number School</span></button>
  ...
  {activeLandmark&&<div className="nt-modal-backdrop" onClick={()=>setActiveLandmark(null)}><section className="nt-landmark-modal" onClick={e=>e.stopPropagation()}><button className="nt-modal-close" aria-label="Close" onClick={()=>setActiveLandmark(null)}><X size={20}/></button><div className="nt-modal-icon">{activeLandmark==="shop"?"🏪":"🌳"}</div><h2>{activeLandmark==="shop"?"Coin Shop":"Number Park"}</h2><p>{activeLandmark==="shop"?"Welcome to the Coin Shop! Rewards and items will be available here.":"Welcome to Number Park! Explore the park and discover number activities."}</p><button className="nt-modal-primary" onClick={()=>setActiveLandmark(null)}>Back to Town</button></section></div>}
  {schoolOpen&&<NumberSchoolModal onClose={()=>setSchoolOpen(false)}/>}</main>;
}
