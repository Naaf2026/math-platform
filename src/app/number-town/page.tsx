...
const PORTAL: Rect = { x: 43, y: 37, w: 14, h: 18 };
const LOCKED_PATH: Rect = { x: 77, y: 72, w: 18, h: 17 };
const OBSTACLES: Rect[] = [
  { x: 7, y: 23, w: 16, h: 17 }, // school
  { x: 79, y: 27, w: 14, h: 15 }, // shop
  { x: 39, y: 67, w: 15, h: 13 }, // park
  { x: 74, y: 67, w: 22, h: 9 }, // locked gate area
];
...
export default function NumberTownPage() {
  ...
  useEffect(() => {
    ...
    const tick = (time: number) => {
      ...
      if (length > 0.08) {
        setPlayer((current) => {
          const proposed: Vec = {
            x: Math.max(2, Math.min(94, current.x + dx * SPEED * dt)),
            y: Math.max(9, Math.min(88, current.y + dy * SPEED * dt)),
          };
          const playerRect: Rect = { x: proposed.x - PLAYER.w / 2, y: proposed.y - PLAYER.h / 2, w: PLAYER.w, h: PLAYER.h };
          if (OBSTACLES.some((obstacle) => intersects(playerRect, obstacle))) return current;
          return proposed;
        });
      }
  ...
  const joystickStart = (event: React.PointerEvent<HTMLDivElement>) => {
  ...
  const updateJoystick = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    joystickRef.current.x = Math.cos(angle) * (length / max);
    joystickRef.current.y = Math.sin(angle) * (length / max);
  };
  const joystickEnd = () => {
  ...
  return <main className="nt-shell">
    ...
    <section className="nt-world">
      ...
      <div className="landmark school">🏫<span>Number School</span></div>
      <div className="landmark shop">🏪<span>Coin Shop</span></div>
      <div className="landmark park">🌳<span>Number Park</span></div>
      ...
      <div className="locked"><Lock size={18}/><span>Logic Valley</span><small>Unlock at Level 2</small></div>
      <div className="town-sign"><MapPin size={18}/> NUMBER TOWN</div>
      ...
      <div className="nt-joystick" onPointerDown={joystickStart} onPointerMove={updateJoystick} onPointerUp={joystickEnd} onPointerCancel={joystickEnd} onPointerLeave={joystickEnd}>
        <div className="nt-joystick-knob" style={{ transform: `translate(${joystickRef.current.x * 35}px, ${joystickRef.current.y * 35}px)` }} />
      </div>
    </section>
