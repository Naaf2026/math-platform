...
export default function TrainingPage() {
  ...
  async function load() {
    ...
    if (profile?.grade) {
      ...
      const match = raw.match(/(?:grade|primary)?\s*([1-7])/i);
      if (match) setGrade(`Grade ${match[1]}`);
    }
    setLoading(false);
  }
  ...
  const topics = useMemo(() => {
    ...
    const derived = questionTopicIds.map((id, index) => ({
      id,
      title: topicTitles[id] || questions.find(q => q.topic_id === id)?.topic || id.replace(/-/g, " "),
      description: "",
      level: grade,
      grade_level: grade,
      lessons: 0,
    ...
    const merged = [...configured.filter(t => !derived.some(d => d.id === t.id)), ...derived];
    return merged.filter(t => t.grade_level === grade || (!t.grade_level && grade === "Grade 3")).sort((a, b) => a.sort_order - b.sort_order);
  }, [allTopics, grade, questions]);

  useEffect(() => {
    if (topics.length && !topics.some(t => t.id === selectedTopic)) setSelectedTopic(topics[0].id);
