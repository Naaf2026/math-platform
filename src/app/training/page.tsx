...
export default function TrainingPage() {
  ...
  async function load() {
    ...
    const [{ data: ts }, { data: qs }, { data: ps }, { data: profile }] = await Promise.all([
      ...
      supabase.rpc("get_adaptive_questions", { p_limit: 200 }),
  ...
  const skillRows = useMemo(() => {
    if (!selected) return [];
    const qs = questions.filter(q => q.topic_id === selected.id || q.topic === selected.id || q.topic?.toLowerCase() === selected.title.toLowerCase() || (selected.id === "place-value" && q.topic?.toLowerCase().includes("place value")));
    return Array.from(new Map(qs.filter(q => q.skill).map(q => [q.skill, q])).values()).slice(0, 12);
  }, [questions, selected]);
  const rows = skillRows.length ? skillRows : [{ id: "fallback-1", skill: selected?.title ?? "Core skill", difficulty: "easy" } as Question];
  ...
  const totalSkills = Math.max(39, rows.length);
  ...
  return (
    <main className="min-h-screen bg-[#f4f4f4] text-[#202020] lg:pl-20">
      <header className="h-12 bg-[#303333] px-4 text-white shadow-md">
        <div className="mx-auto flex h-full max-w-[1400px] items-center justify-between">
          <div className="flex items-center gap-4 font-bold"><div className="flex items-center gap-2 text-lg tracking-tight"><span className="grid h-8 w-8 place-items-center rounded-md bg-white text-[11px] font-black text-[#303333]">FV</span><span>FAHI VISSNUN</span></div><span className="rounded-full bg-[#1f2525] px-4 py-1 text-xs">Math ▾</span></div>
          <nav className="hidden items-center gap-10 text-sm font-semibold md:flex"><Link href="/dashboard" className="flex items-center gap-2 hover:text-yellow-300"><Home size={17}/>Home</Link><Link href="/progress" className="flex items-center gap-2 hover:text-yellow-300"><BarChart3 size={17}/>Report</Link><Link href="/leaderboard" className="flex items-center gap-2 hover:text-yellow-300"><Trophy size={17}/>Leaderboard</Link><Link href="/rewards" className="flex items-center gap-2 hover:text-yellow-300"><Award size={17}/>Achievement</Link><span className="flex items-center gap-2"><HelpCircle size={18}/>Help</span></nav>
          <span className="grid h-8 w-8 place-items-center rounded-full bg-[#555]">⌄</span>
        </div>
      </header>

      <section className="relative h-[105px] overflow-hidden bg-[#103dd2] text-white">
        <div className="absolute inset-0 opacity-80" style={{ backgroundImage: "radial-gradient(circle, rgba(255,255,255,.9) 1px, transparent 1.5px)", backgroundSize: "28px 24px" }} />
        <div className="relative mx-auto flex h-full max-w-[900px] items-center justify-center gap-7 text-center"><span className="text-4xl">🚀</span><div><p className="text-xl font-black sm:text-2xl">Click Practice Button to Start!</p><p className="text-xs font-bold text-blue-100">(Total {totalSkills} skills)</p></div><span className="text-2xl">★</span></div>
      </section>
