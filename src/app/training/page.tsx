...
import { ArrowRight, BookOpen, CheckCircle2, ChevronRight, LockKeyhole, Sparkles, Target, Trophy, Zap } from "lucide-react";
...
const topicMeta: Record<string, { emoji: string; tone: string; soft: string }> = {
  "place-value": { emoji: "🔢", tone: "from-violet-500 to-indigo-500", soft: "bg-violet-50" },
  "addition-subtraction": { emoji: "➕", tone: "from-cyan-400 to-blue-500", soft: "bg-cyan-50" },
  multiplication: { emoji: "✖️", tone: "from-orange-400 to-amber-500", soft: "bg-orange-50" },
  fractions: { emoji: "🍕", tone: "from-emerald-400 to-teal-500", soft: "bg-emerald-50" },
};
...
function meta(id: string) { return topicMeta[id] ?? { emoji: "📘", tone: "from-blue-500 to-cyan-500", soft: "bg-blue-50" }; }
  ...
  return <main className="min-h-screen bg-gradient-to-br from-[#f6f7ff] via-white to-[#eefbff] pb-28 lg:pl-20">
    ...
    <div className="mx-auto max-w-7xl px-5 py-6 lg:px-8 lg:py-8">
      ...
      {!selectedTopic ? <>
        ...
        <section className="mt-6 rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm sm:p-7"><div className="flex items-center justify-between"><div><p className="text-xs font-black uppercase tracking-wider text-violet-600">Skills</p><h3 className="mt-1 text-2xl font-black text-[#15233f]">Pick a skill to practise</h3></div><span className="rounded-full bg-violet-50 px-3 py-1.5 text-xs font-black text-violet-700">{visibleQuestions.length} questions available</span></div>{skills.length ? <div className="mt-5 grid gap-3 sm:grid-cols-2">{skills.map(q => <div key={q.skill} className="flex items-center gap-4 rounded-2xl border-2 border-slate-100 p-4 transition hover:border-violet-200 hover:bg-violet-50/40"><div className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-violet-100 text-violet-600"><CheckCircle2 size={21}/></div><div className="min-w-0 flex-1"><p className="font-black text-[#15233f]">{q.skill}</p><p className="mt-1 text-xs font-bold text-slate-400">{difficultyLabel(q.difficulty)} · Practice questions</p></div><span className="text-slate-300"><ChevronRight size={20}/></span></div>)}</div> : <div className="mt-5 rounded-3xl bg-slate-50 p-7 text-center"><p className="font-black text-slate-700">Skills are being added to this topic.</p><p className="mt-1 text-sm text-slate-500">Use the topic mission below while the skill bank grows.</p></div>}
          <div className="mt-7 border-t border-dashed border-slate-200 pt-6"><div className="flex flex-col gap-4 rounded-3xl bg-gradient-to-r from-violet-50 to-cyan-50 p-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-xs font-black uppercase tracking-wider text-violet-600">10-question practice</p><p className="mt-1 font-black text-[#15233f]">Ready to train {selected?.title}?</p><p className="mt-1 text-sm font-semibold text-slate-500">Launch the adaptive practice experience and keep your results in your account.</p></div><Link href="/mission" className="inline-flex shrink-0 items-center justify-center gap-2 rounded-2xl bg-violet-600 px-6 py-3.5 font-black text-white shadow-lg hover:bg-violet-700">Start practice <ArrowRight size={17}/></Link></div></div></section>
