...
export default function TrainingPracticePage() {
  return <Suspense fallback={<main className="grid min-h-screen place-items-center bg-[#f5f7ff] p-6"><p className="font-bold text-[#15233f]">Preparing practice…</p></main>}><TrainingPracticeContent/></Suspense>;
}

function TrainingPracticeContent() {
  ...
  async function load() {
    ...
    const { data: learner, error: learnerError } = await supabase.from("profiles").select("grade,xp").eq("id",auth.user.id).maybeSingle();
    if (learnerError) { setError("Your grade could not be loaded."); setLoading(false); return; }
    const actualGrade = String(learner?.grade ?? "").match(/(?:grade|primary)?\s*([1-5])/i);
    if (!actualGrade) { setError("Set your grade in your profile before starting practice."); setLoading(false); return; }
    const learnerGrade = `Grade ${actualGrade[1]}`;
    if (requestedGrade && requestedGrade !== learnerGrade) { setError("This practice belongs to a different grade."); setLoading(false); return; }
    const { data: qs, error: qError } = await supabase.rpc("get_training_questions_by_grade", {
        p_grade_level: learnerGrade,
  ...
  const title = requestedSkill || q?.skill || q?.topic || "Math Practice";
  ...
  if (error || !questions.length) return <main className="min-h-screen bg-[#f5f7ff] p-6"><div className="mx-auto grid min-h-[80vh] max-w-lg place-items-center"><div className="w-full rounded-[2rem] bg-white p-9 text-center shadow-xl"><XCircle className="mx-auto text-rose-500" size={44}/><h1 className="mt-4 text-2xl font-black">Practice is not ready</h1><p className="mt-2 text-sm font-bold text-slate-500">{error || `There are no questions for this skill yet.`}</p><Link href="/learn" className="mt-6 inline-flex rounded-2xl bg-violet-600 px-6 py-3 font-black text-white">Back to Learn</Link></div></div></main>;
