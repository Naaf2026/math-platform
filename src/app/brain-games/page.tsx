...
export default function BrainGamesPage() {
  ...
  return <main className="min-h-screen overflow-x-hidden bg-[#05b8d8]">
    <section className="relative min-h-screen overflow-hidden px-3 pb-5 pt-3 md:hidden">
      ...
      <div className="relative z-10 mx-auto max-w-md">
        <div className="mb-3 flex items-center justify-between rounded-2xl border border-white/60 bg-[#062b57]/85 px-3 py-2 text-white shadow-xl backdrop-blur-md">
          <Link href="/" aria-label="Home" className="inline-flex h-8 shrink-0 items-center gap-1.5 rounded-full bg-white/20 px-2.5 text-[10px] font-black text-white ring-1 ring-white/30 transition active:scale-95"><House size={14} /> Home</Link>
    ...
    <section className="relative mx-auto hidden w-full max-w-[1800px] md:block"><div className="relative aspect-[1672/940] w-full">
      ...
      <Link href="/" aria-label="Home" className="absolute bottom-[3.5%] left-[1.5%] z-20 inline-flex h-[5%] min-h-9 w-auto min-w-9 items-center justify-center gap-1.5 rounded-full border-2 border-white/80 bg-[#062b57]/80 px-3 text-white shadow-lg backdrop-blur transition hover:scale-105 hover:bg-[#062b57]"><House size={15} /><span className="text-[10px] font-black uppercase tracking-wide">Home</span></Link>
    </div></section>
  </main>;
}
