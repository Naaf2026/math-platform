    ...
    <main className="min-h-screen overflow-x-hidden bg-[#eef8ff] px-3 py-2 text-[#17395f] sm:px-5 sm:py-3">
      <div className="mx-auto max-w-6xl">
        ...
        <section className="relative px-1 pb-3 pt-1 text-center sm:pb-4 sm:pt-1.5">
          <div className="pointer-events-none absolute left-[15%] top-1 text-lg text-pink-400 sm:text-xl">✦</div>
          <div className="pointer-events-none absolute right-[15%] top-3 text-sm text-amber-400 sm:text-lg">✦</div>
        ...
        <nav className="flex gap-1.5 overflow-x-auto border-y border-sky-100 py-1.5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {categories.map(([icon, title, href]) => (
            <Link key={href} href={`/brain-games/${href}`} className={`flex shrink-0 items-center gap-1 rounded-full border px-2.5 py-1 text-[9px] font-black sm:text-[10px] ${href === "memory" ? "border-violet-400 bg-violet-500 text-white shadow-sm" : "border-transparent bg-white text-slate-500 hover:border-sky-100"}`}><span>{icon}</span>{title}</Link>
          ))}
        </nav>
        ...
        <section className="mt-2.5">
          ...
          <div className="mb-1.5 flex items-center justify-between px-1"><h2 className="text-sm font-black text-slate-800 sm:text-base">Choose your memory challenge 🎮</h2><span className="text-[8px] font-black text-violet-400">4 GAMES</span></div>

          <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-4">
            {games.map((game) => (
              <article key={game.title} className={`group overflow-hidden rounded-[22px] border border-white bg-white p-1.5 shadow-[0_5px_18px_rgba(15,23,42,.08)] transition duration-200 ${game.active ? "hover:-translate-y-1 hover:shadow-[0_12px_24px_rgba(15,23,42,.12)]" : "opacity-90"}`}>
                ...
                <div className="px-1 pt-2">
                  <div className="flex items-center justify-between gap-1"><h3 className="truncate text-[13px] font-black text-slate-800 sm:text-sm">{game.title}</h3>{!game.active && <Lock size={11} className="shrink-0 text-slate-300" />}</div>
                  <p className="mt-0.5 min-h-[25px] text-[9px] font-bold leading-3 text-slate-400 sm:text-[10px]">{game.description}</p>
                  <div className="mt-1 flex items-center justify-between"><span className="text-[8px] font-black text-amber-400">{game.difficulty}</span><span className="text-[8px] font-black text-violet-400">{game.reward}</span></div>
                  {game.active ? <Link href={game.href} className="mt-1.5 inline-flex w-full items-center justify-center gap-1 rounded-xl bg-violet-500 px-2 py-2 text-[10px] font-black text-white shadow-sm transition hover:bg-violet-600 hover:scale-[1.01]"><Play size={10} fill="currentColor" /> PLAY NOW</Link> : <span className="mt-1.5 inline-flex w-full items-center justify-center rounded-xl bg-slate-100 px-2 py-2 text-[9px] font-black text-slate-400">Coming Soon</span>}
                </div>
              </article>
            ))}
          </div>
