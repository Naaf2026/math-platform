  ...
  <section className="mt-5 print:mt-0">
   ...
   <div className="mt-5 space-y-4">{problems.map((p,i)=>{const expected=answerFor(p,operation),correct=(answers[i]??"")===expected;return <div key={i} className="grid items-center gap-4 rounded-2xl border-2 border-[#cde6e3] bg-white p-4 sm:grid-cols-[1fr_auto] print:break-inside-avoid">
    <div className="flex min-w-0 flex-wrap items-center gap-3">
     {operation==="Addition"&&<><div className="rounded-2xl bg-[#eefaf7] px-4 py-3 text-xl leading-loose sm:text-2xl">{objects(p.icon,p.a)} <b className="mx-2 text-cyan-700">+</b> {objects(p.icon,p.b)}</div><p className="whitespace-nowrap text-xl font-black sm:text-2xl">{i+1}. {p.a} + {p.b} =</p></>}
     {operation==="Subtraction"&&<><div className="rounded-2xl bg-[#eefaf7] px-4 py-3 text-xl leading-loose sm:text-2xl">{objects(p.icon,p.a)} <b className="mx-2 text-cyan-700">−</b> {objects(p.icon,p.b)}</div><p className="whitespace-nowrap text-xl font-black sm:text-2xl">{i+1}. {p.a} − {p.b} =</p></>}
     {operation==="Counting"&&<><div className="rounded-2xl bg-[#eefaf7] px-4 py-3 text-xl leading-loose sm:text-2xl">{objects(p.icon,p.a)}</div><p className="text-xl font-black sm:text-2xl">{i+1}. How many?</p></>}
     {operation==="Comparing Numbers"&&<p className="text-2xl font-black sm:text-3xl">{i+1}. <span className="rounded-xl bg-cyan-50 px-4 py-2">{p.a}</span> <span className="mx-2 text-slate-400">?</span> <span className="rounded-xl bg-cyan-50 px-4 py-2">{p.b}</span></p>}
    </div>
    <div className="flex items-center gap-2">{operation==="Comparing Numbers"?<div className="flex gap-2">{[">","<","="].map(v=><button key={v} disabled={checked} onClick={()=>setAnswer(i,v)} className={`h-14 w-14 rounded-xl border-2 text-2xl font-black ${answers[i]===v?"border-violet-600 bg-violet-50":"border-violet-200"}`}>{v}</button>)}</div>:<input aria-label={`Answer for question ${i+1}`} inputMode="numeric" disabled={checked} value={answers[i]??""} onChange={e=>setAnswer(i,e.target.value.replace(/\D/g,""))} className="h-16 w-20 rounded-xl border-[3px] border-violet-200 bg-white text-center text-2xl font-black outline-none focus:border-violet-500 print:border-slate-500"/>}{checked&&(correct?<CheckCircle2 className="text-emerald-600 print:hidden" size={24}/>:<span className="flex items-center gap-1 font-black text-rose-600 print:hidden"><XCircle size={24}/><span className="text-sm">{expected}</span></span>)}</div>
   </div>})}</div>
