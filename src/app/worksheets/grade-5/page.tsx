...
function make(topic:string,i:number,seed:number,challenge=1):Item{
 ...
 case "Adding fractions":{const den=5+n%19,p=1+Math.floor(n/19)%4,q=1+Math.floor(n/76)%4;return{q:`Calculate ${p}/${den} + ${q}/${den}. Give your answer as a fraction.`,a:`${p+q}/${den}`,hint:"For like denominators, add the numerators."}}
 case "Decimals":{const v=(x*10+y)/10;return{q:`${v.toFixed(1)} + ${(z/10).toFixed(1)} = ?`,a:(v+z/10).toFixed(1),hint:"Line up the decimal points."}}
 case "Percentages":{const pct=[10,20,25,50][n%4],whole=(20+Math.floor(n/4)%150)*20;return{q:`What is ${pct}% of ${whole}?`,a:String(pct*whole/100),hint:"Percent means per hundred."}}
 ...
 case "Angles":{const v=10+(n*17)%171;return{q:`Classify a ${v}° angle: acute, right, obtuse or straight.`,a:v===90?"right":v===180?"straight":v<90?"acute":"obtuse",hint:"Acute < 90°, right = 90°, obtuse between 90° and 180°."}}
 case "2-D shapes":{const k=n%5,amount=1+Math.floor(n/5)%20;return{q:`How many sides altogether do ${amount} separate ${["triangles","quadrilaterals","pentagons","hexagons","octagons"][k]} have?`,a:String(amount*[3,4,5,6,8][k]),hint:"Count the sides of one shape, then multiply."}}
 case "Symmetry":{const shapes=["square","rectangle","equilateral triangle","regular pentagon","regular hexagon","isosceles triangle","regular octagon"];const k=n%shapes.length,amount=1+Math.floor(n/7)%20;return{q:`How many lines of symmetry altogether do ${amount} separate ${["squares","rectangles","equilateral triangles","regular pentagons","regular hexagons","isosceles triangles","regular octagons"][k]} have?`,a:String([4,2,3,5,6,1,8][k]*amount),hint:"Count the lines on one shape, then multiply."}}
 case "3-D shapes":{const k=n%4;const names=["cubes","triangular prisms","square-based pyramids","rectangular prisms"],faces=[6,5,5,6],edges=[12,9,8,12],vertices=[8,6,5,8];const mode=Math.floor(n/4)%3,amount=1+Math.floor(n/12)%20;return{q:`How many ${["faces","edges","vertices"][mode]} altogether do ${amount} separate ${names[k]} have?`,a:String([faces,edges,vertices][mode][k]*amount),hint:"Count one solid, then multiply by the number of solids."}}
 ...
 case "Probability":{const red=2+n%19,blue=3+Math.floor(n/19)%17;return{q:`A bag has ${red} red and ${blue} blue counters. What is the probability of picking a red counter? Answer as a fraction.`,a:`${red}/${red+blue}`,hint:"Favourable outcomes / total outcomes. An unsimplified fraction is accepted."}}
 ...
 case "Lowest common multiple":{const u=2+n%19,v=2+Math.floor(n/19)%17;let k=Math.max(u,v);while(k%u!==0||k%v!==0)k++;return{q:`Find the lowest common multiple (LCM) of ${u} and ${v}.`,a:String(k),hint:"Find the smallest positive number divisible by both."}}
