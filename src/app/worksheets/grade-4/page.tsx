...
switch(topic){
case "Counting backwards through zero":{const step=1+n%5,start=step*(2+n%5),term=start-step*(3+Math.floor(n/5)%5);return{q:`Count backwards by ${step}: ${start}, ${start-step}, ${start-2*step}, ___ .`,a:String(term),hint:"Subtract the same step each time, even when you pass zero."}}
...
case "Fractions":{const den=[2,3,4,5,8,10][n%6],num=1+Math.floor(n/6)%(den-1),multiple=2+Math.floor(n/36)%30;return{q:`What is ${num}/${den} of ${den*multiple}?`,a:String(num*multiple),hint:"Divide by the denominator, then multiply by the numerator."}}
...
case "3-D shapes":{const names=["cube","cuboid","triangular prism","square-based pyramid"],features=[[6,12,8],[6,12,8],[5,9,6],[5,8,5]],k=n%4,mode=Math.floor(n/4)%3;return{q:`How many ${["faces","edges","vertices"][mode]} does a ${names[k]} have?`,a:String(features[k][mode]),hint:"Count the requested features of the solid."}}
