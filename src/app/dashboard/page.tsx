...
import { Bell, BarChart3, Gamepad2, GraduationCap, Home, LogOut, Trophy, Gift, LogIn, UserRound } from "lucide-react";
import { createClient } from "@/lib/supabase/client";
import LearnerLoginModal from "@/components/auth/LearnerLoginModal";

type Profile = { full_name: string | null; grade: string | null; xp: number; current_streak: number; is_premium: boolean };
type MindStatus = { balance: number; remaining_seconds: number };

function VisualMathIllustration() {
  return (
    <svg viewBox="0 0 320 190" className="h-full w-full" role="img" aria-label="Visual maths blocks and shapes">
      <defs>
        <linearGradient id="lab-sky" x1="0" x2="1">
          <stop offset="0" stopColor="#6c5ce7" />
          <stop offset="1" stopColor="#24b8d6" />
        </linearGradient>
      </defs>
      <rect width="320" height="190" rx="28" fill="url(#lab-sky)" />
      <circle cx="260" cy="42" r="24" fill="#fef08a" opacity=".9" />
      <path d="M38 145h244" stroke="#fff" strokeWidth="5" strokeLinecap="round" opacity=".35" />
      <rect x="53" y="93" width="45" height="52" rx="7" fill="#8b7cff" />
      <rect x="103" y="76" width="45" height="69" rx="7" fill="#67e8f9" />
      <rect x="153" y="58" width="45" height="87" rx="7" fill="#fcd34d" />
      <text x="75" y="164" textAnchor="middle" fill="#fff" fontSize="13" fontWeight="800">100</text>
      <text x="125" y="164" textAnchor="middle" fill="#fff" fontSize="13" fontWeight="800">10</text>
      <text x="175" y="164" textAnchor="middle" fill="#fff" fontSize="13" fontWeight="800">1</text>
      <path d="M224 105l14 14 27-31" fill="none" stroke="#fff" strokeWidth="8" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (!mounted) return;
      setProfile(data ?? { full_name: auth.user.user_metadata?.full_name ?? "Student", grade: null, xp: 0, current_streak: 0, is_premium: false });
      if (mindData) {
    ...
    <main className="min-h-screen overflow-x-hidden bg-[#eef9ff] text-[#083d78]">
      <header className="sticky top-0 z-40 h-[76px] border-b border-white/10 bg-[#073b73] text-white shadow-sm lg:h-[90px]">
        <div className="mx-auto flex h-full max-w-[1680px] items-center justify-between px-4 sm:px-6 lg:px-12">
          ...
          {loggedOut ? (
            ...
            <div className="flex items-center gap-2 sm:gap-3"><nav className="hidden items-center gap-5 lg:flex"><Link href="/dashboard" className="relative flex items-center gap-2 px-3 py-7 text-lg font-black"><Home size={23} /> Home<span className="absolute bottom-0 left-3 right-3 h-1 rounded-full bg-yellow-400" /></Link><Link href="/brain-games" className="flex items-center gap-2 px-3 py-7 text-lg font-bold hover:text-yellow-200"><Gamepad2 size={23} /> Games</Link><Link href="/leaderboard" className="flex items-center gap-2 px-3 py-7 text-lg font-bold hover:text-yellow-200"><Trophy size={23} /> Leaderboard</Link><Link href="/rewards" className="flex items-center gap-2 px-3 py-7 text-lg font-bold hover:text-yellow-200"><Gift size={23} /> Rewards</Link><Link href="/progress" className="flex items-center gap-2 px-3 py-7 text-lg font-bold hover:text-yellow-200"><BarChart3 size={23} /> Progress</Link><Link href="/profile" className="flex items-center gap-2 px-3 py-7 text-lg font-bold hover:text-yellow-200"><UserRound size={23} /> Profile</Link></nav><div className="flex items-center gap-2 sm:gap-3"><div className={`hidden rounded-full px-3 py-1.5 text-sm font-black sm:block ${profile.is_premium ? "bg-yellow-300 text-[#073b73]" : "bg-white/10 text-white"}`}>{profile.is_premium ? "Premium 👑" : "Upgrade to Premium → 👑"}</div><div className="hidden items-center gap-3 rounded-full bg-white/10 px-3 py-1.5 xl:flex"><span className="text-sm font-black">✨ {mind.balance}</span><span className="text-sm font-black">⏱️ Mind Time {formatMindTime(mind.remaining_seconds)}</span></div><button aria-label="Notifications" className="relative grid h-11 w-11 place-items-center rounded-full hover:bg-white/10 sm:h-12 sm:w-12"><Bell size={24} /><span className="absolute right-1.5 top-1.5 h-2.5 w-2.5 rounded-full bg-red-400" /></button></div></div>
          )}
        </div>
      </header>
      ...
      {!loggedOut && <nav className="fixed inset-x-0 bottom-0 z-50 border-t border-[#cfe4f2] bg-white/95 px-2 pb-[calc(env(safe-area-inset-bottom)+6px)] pt-2 shadow-[0_-6px_20px_rgba(8,61,120,0.10)] backdrop-blur lg:hidden" aria-label="Student navigation">
        <div className="mx-auto grid max-w-lg grid-cols-6">
          ...
          <Link href="/profile" className="flex flex-col items-center justify-center gap-1 rounded-xl py-1.5 text-[#526f89]">
            <UserRound size={22} strokeWidth={2.5} /><span className="text-[11px] font-black">Profile</span>
          </Link>
        </div>
      </nav>}
