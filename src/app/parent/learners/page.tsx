...
export default function ParentLearnersPage() {
  ...
  useEffect(() => {
    ...
    async function checkPhone() {
      ...
      const { data, error: profileError } = await supabase.from("profiles").select("mobile_phone").eq("id", user.id).single();
  ...
  async function submit(e: FormEvent<HTMLFormElement>) {
    e.preventDefault();
    if (!phoneReady) { setStatus("Add your mobile phone number in Parent Profile before registering a learner."); return; }
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-violet-50 via-white to-cyan-50 p-5 pb-24 sm:p-8">
      <div className="mx-auto max-w-5xl">
        ...
        {showForm && phoneReady === false && <section className="mt-6 rounded-3xl border border-amber-200 bg-amber-50 p-6 sm:p-8"><h2 className="text-xl font-black text-[#071b3a]">Add your mobile number first</h2><p className="mt-2 text-sm leading-6 text-slate-700">A parent mobile phone number is required before you can register a learner.</p><Link href="/parent/profile" className="mt-4 inline-flex rounded-xl bg-[#071b3a] px-5 py-3 font-black text-white">Go to Parent Profile</Link></section>}
