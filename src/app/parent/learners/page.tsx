...
export default function ParentLearnersPage() {
  ...
  const [phoneReady, setPhoneReady] = useState<boolean | null>(null);
  ...
  useEffect(() => {
    ...
    async function checkPhone() {
      ...
      if (!supabase) { setPhoneReady(false); return; }
      const { data: { user }, error: authError } = await supabase.auth.getUser();
      if (authError || !user) { setPhoneReady(false); return; }
      const { data, error: profileError } = await supabase.from("profiles").select("mobile_phone, full_name").eq("id", user.id).single();
      setParentName(data?.full_name || user.user_metadata?.full_name || user.user_metadata?.display_name || "");
      if (data?.mobile_phone) setMobilePhone(data.mobile_phone);
      setPhoneReady(!profileError && /^\+[1-9]\d{7,14}$/.test(data?.mobile_phone || ""));
    }
    void checkPhone();
  }, []);
  ...
  async function submit(e: FormEvent<HTMLFormElement>) {
    e.preventDefault();
    if (!school || (school === "other" && !otherSchool.trim())) { setStatus("Please select or enter the learner’s school."); return; }
    ...
    try {
      ...
      const result = await createLearnerAccount({ display_name: name, username, password, grade, school_name: school === "other" ? otherSchool.trim() : school, avatar_emoji: avatar });
      setCredentials({ username: result.username, password });
      setName(""); setUsername(""); setPassword(""); setGrade(""); setSchool(""); setOtherSchool(""); setAvatar("🧑‍🎓"); setShowForm(false);
      await load();
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-violet-50 via-white to-cyan-50 p-5 pb-24 sm:p-8">
      <div className="mx-auto max-w-5xl">
        ...
        {showForm && phoneReady === null && <p className="mt-6 rounded-2xl bg-white p-5 text-sm font-bold text-slate-500">Checking your parent profile…</p>}
        {showForm && phoneReady !== null && <section className="mt-6 rounded-3xl bg-white p-6 shadow-sm ring-1 ring-violet-100 sm:p-8"><div className="flex items-center gap-3"><GraduationCap className="text-violet-600" /><div><h2 className="text-2xl font-black text-[#071b3a]">Add a learner</h2><p className="text-sm text-slate-500">Learners do not sign up themselves.</p></div></div><form onSubmit={submit} className="mt-6 grid gap-4 sm:grid-cols-2">{phoneReady === false && <label className="block sm:col-span-2"><span className="text-sm font-bold text-slate-700">Parent mobile phone number</span><input required type="tel" autoComplete="tel" inputMode="tel" value={mobilePhone} onChange={e => setMobilePhone(e.target.value)} className="mt-1.5 w-full rounded-xl border border-slate-200 px-4 py-3 outline-none focus:border-violet-500" placeholder="+9607777777" /><span className="mt-1 block text-xs text-slate-500">Required when adding your first learner. Include your country code.</span></label>}<label className="block"><span className="text-sm font-bold text-slate-700">Learner name</span><input required value={name} onChange={e => setName(e.target.value)} className="mt-1.5 w-full rounded-xl border border-slate-200 px-4 py-3 outline-none focus:border-violet-500" placeholder="Child's name" /></label><label className="block"><span className="text-sm font-bold text-slate-700">Username</span><input required pattern="[a-z0-9][a-z0-9._-]{2,31}" value={username} onChange={e => setUsername(e.target.value.toLowerCase())} className="mt-1.5 w-full rounded-xl border border-slate-200 px-4 py-3 outline-none focus:border-violet-500" placeholder="e.g. ali2026" /><span className="mt-1 block text-xs text-slate-400">3–32 lowercase letters, numbers, . _ or -</span></label><label className="block"><span className="text-sm font-bold text-slate-700">Password</span><input required minLength={6} type="password" value={password} onChange={e => setPassword(e.target.value)} className="mt-1.5 w-full rounded-xl border border-slate-200 px-4 py-3 outline-none focus:border-violet-500" placeholder="At least 6 characters" /></label><label className="block"><span className="text-sm font-bold text-slate-700">Grade</span><select value={grade} onChange={e => setGrade(e.target.value)} className="mt-1.5 w-full rounded-xl border border-slate-200 bg-white px-4 py-3"><option value="">Select grade</option>{grades.map(g => <option key={g}>{g}</option>)}</select></label><label className="block sm:col-span-2"><span className="text-sm font-bold text-slate-700">School</span><select required value={school} onChange={e => { setSchool(e.target.value); setOtherSchool(""); }} className="mt-1.5 w-full rounded-xl border border-slate-200 bg-white px-4 py-3 outline-none focus:border-violet-500"><option value="">Select school</option>{maldivesSchoolGroups.map(group => <optgroup key={group.atoll} label={group.atoll}>{group.schools.map(name => <option key={name} value={`${group.atoll} / ${name}`}>{name}</option>)}</optgroup>)}<option value="other">Other school (enter name)</option></select><span className="mt-1 block text-xs text-slate-400">Schools are grouped by atoll. Choose Other school if yours is not listed.</span></label>{school === "other" && <label className="block sm:col-span-2"><span className="text-sm font-bold text-slate-700">School name</span><input required maxLength={120} value={otherSchool} onChange={e => setOtherSchool(e.target.value)} className="mt-1.5 w-full rounded-xl border border-slate-200 px-4 py-3 outline-none focus:border-violet-500" placeholder="Enter your child’s school" /></label>}<div className="sm:col-span-2"><span className="text-sm font-bold text-slate-700">Avatar</span><div className="mt-2 flex flex-wrap gap-2">{avatars.map(a => <button type="button" key={a} onClick={() => setAvatar(a)} className={`rounded-xl px-3 py-2 text-2xl ${avatar === a ? "bg-violet-100 ring-2 ring-violet-500" : "bg-slate-50"}`}>{a}</button>)}</div></div>{status && <p role="alert" className="sm:col-span-2 rounded-xl bg-amber-50 p-3 text-sm text-amber-800">{status}</p>}<div className="sm:col-span-2 flex flex-wrap gap-3"><button disabled={busy} className="inline-flex items-center gap-2 rounded-xl bg-[#071b3a] px-5 py-3 font-black text-white disabled:opacity-60">{busy && <Loader2 className="animate-spin" size={17} />}{busy ? "Creating learner…" : "Create learner account"}</button><button type="button" onClick={() => setShowForm(false)} className="rounded-xl bg-slate-100 px-5 py-3 font-bold text-slate-600">Cancel</button></div></form></section>}
