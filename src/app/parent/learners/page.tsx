...
import { ArrowLeft, CheckCircle2, Copy, GraduationCap, Loader2, Plus, ShieldCheck } from "lucide-react";
import { createLearnerAccount, getMyLearners, type LearnerAccount } from "@/lib/parent-learners";
    ...
    <main className="min-h-screen bg-gradient-to-br from-violet-50 via-white to-cyan-50 p-5 pb-24 sm:p-8">
      <div className="mx-auto max-w-5xl">
        ...
        <section className="mt-7 rounded-[2rem] bg-gradient-to-br from-violet-700 via-indigo-600 to-blue-500 p-7 text-white shadow-2xl sm:p-9">
          <div className="flex items-start gap-4"><div className="rounded-2xl bg-white/15 p-3"><ShieldCheck size={25} /></div><div><p className="text-xs font-black uppercase tracking-[0.18em] text-indigo-100">Family accounts</p><h1 className="mt-2 text-3xl font-black sm:text-4xl">Your learners</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-indigo-100">Create a secure learner account for each child. You choose their username and password, then give those credentials to them for learner sign-in.</p></div></div>
        </section>
        ...
        <section className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{learners.map(learner => <article key={learner.learner_id} className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-100"><div className="flex items-center gap-3"><div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-violet-50 text-3xl">{learner.avatar_emoji || "🧑‍🎓"}</div><div><h2 className="font-black text-[#071b3a]">{learner.display_name}</h2><p className="text-sm font-bold text-violet-600">@{learner.username}</p></div></div><div className="mt-5 grid grid-cols-2 gap-2 text-sm"><div className="rounded-xl bg-slate-50 p-3"><span className="text-xs text-slate-400">Grade</span><p className="font-bold">{learner.grade || "Not set"}</p></div><div className="rounded-xl bg-slate-50 p-3"><span className="text-xs text-slate-400">XP</span><p className="font-bold">{learner.xp}</p></div></div><p className="mt-4 text-xs text-slate-400">Streak: {learner.current_streak} days · Best: {learner.best_streak} days</p></article>)}{learners.length === 0 && <div className="rounded-3xl bg-white p-8 text-center shadow-sm sm:col-span-2 lg:col-span-3"><GraduationCap className="mx-auto text-violet-500" size={40} /><h2 className="mt-3 text-xl font-black text-[#071b3a]">No learner accounts yet</h2><p className="mt-2 text-sm text-slate-500">Add your first learner to get started.</p><button onClick={() => setShowForm(true)} className="mt-5 rounded-xl bg-[#071b3a] px-5 py-3 font-black text-white">Add first learner</button></div>}</section>
      </div>
    </main>
  );
}
