...
import { ArrowLeft, CheckCircle2, Plus, Target, TrendingUp } from "lucide-react";
...
export default function ParentGoalsPage() {
  ...
  const load = useCallback(async () => {
    ...
    setStudentId(id);
    const { data, error: goalError } = await s.rpc("get_parent_learning_goals", { p_student_id: id });
  ...
  async function createGoal(e: React.FormEvent) {
    ...
    if (!form.title.trim() || !Number.isFinite(target) || target <= 0 || !form.due_date) {
      setError("Enter a title, positive target and due date.");
      return;
    }
    const s = createClient();
    if (!s) return;
    ...
    const { error: saveError } = await s.rpc("create_parent_learning_goal", {
      ...
      p_topic_title: form.topic_title.trim() || null,
    ...
    if (saveError) setError(saveError.message);
      ...
      setForm({ title: "", description: "", goal_type: "questions", target_value: "20", topic_title: "", due_date: "" });
      await load();
    }
    setSaving(false);
  }
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <section className="mt-7 rounded-[2rem] bg-gradient-to-br from-violet-700 via-indigo-600 to-blue-500 p-7 text-white shadow-2xl sm:p-10">
          <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <div className="inline-flex items-center gap-2 rounded-full bg-white/15 px-3 py-1.5 text-xs font-black uppercase tracking-wider"><Target size={14}/> Family goals</div>
              <h1 className="mt-4 text-4xl font-black tracking-tight sm:text-5xl">Goal management center</h1>
              <p className="mt-3 max-w-2xl text-indigo-100">Create measurable learning goals and keep every active target in one focused place.</p>
            </div>
          ...
          <section className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-violet-100 sm:p-7">
            ...
            <form onSubmit={createGoal} className="mt-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <select value={form.goal_type} onChange={e => setForm(v => ({ ...v, goal_type: e.target.value }))} className="rounded-2xl border border-slate-200 px-4 py-3 text-sm font-bold"><option value="questions">Questions</option><option value="accuracy">Accuracy</option><option value="xp">XP</option><option value="practice_days">Practice days</option></select>
                <input type="number" min="1" value={form.target_value} onChange={e => setForm(v => ({ ...v, target_value: e.target.value }))} className="rounded-2xl border border-slate-200 px-4 py-3 text-sm font-bold" placeholder="Target" />
              </div>
