...
export default function ParentPage() {
  ...
  const eligibleTopics = useMemo(() => [...topics]
    .filter((topic) => Number(topic.questions_answered || 0) >= MIN_TOPIC_ATTEMPTS)
    .sort((a, b) => Number(b.mastery) - Number(a.mastery)), [topics]);

  const strongest = useMemo(() => {
    if (eligibleTopics.length) return eligibleTopics.slice(0, 3);
    return [...topics].sort((a, b) => Number(a.topic_id.localeCompare(b.topic_id))).slice(0, 3);
  }, [eligibleTopics, topics]);

  const support = useMemo(() => {
    if (eligibleTopics.length) return [...eligibleTopics].sort((a, b) => Number(a.mastery) - Number(b.mastery)).slice(0, 3);
    return [...topics].sort((a, b) => Number(a.topic_id.localeCompare(b.topic_id))).slice(0, 3);
  }, [eligibleTopics, topics]);
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-violet-50 via-white to-cyan-50 p-5 pb-28 sm:p-8"><div className="mx-auto max-w-6xl">
      ...
      <section className="mt-6 grid gap-6 lg:grid-cols-2"><TopicPanel title="Strongest areas" eyebrow="Learning strengths" icon={<Medal/>} topics={strongest} empty="Complete more practice to reveal the strongest learning areas."/><TopicPanel title="Areas to support" eyebrow="Suggested focus" icon={<TrendingUp/>} topics={support} empty="Complete more practice to reveal where extra support is useful."/></section>
...
function TopicPanel({title,eyebrow,icon,topics,empty}:{title:string;eyebrow:string;icon:React.ReactNode;topics:Topic[];empty:string}){return <section className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-violet-100 sm:p-8"><div className="flex items-center gap-3"><div className="rounded-2xl bg-violet-50 p-3 text-violet-700">{icon}</div><div><p className="text-xs font-black uppercase tracking-wider text-violet-600">{eyebrow}</p><h2 className="text-2xl font-black text-[#071b3a]">{title}</h2></div></div>{topics.length ? <div className="mt-6 space-y-4">{topics.map((topic)=>{const questions=Number(topic.questions_answered||0);const correct=Number(topic.correct_answers||0);const mastery=Math.min(100,Math.max(0,Number(topic.mastery||0)));return <div key={topic.topic_id} className="rounded-2xl border border-slate-100 bg-white p-4"><div className="flex items-center justify-between gap-3"><div className="min-w-0"><p className="truncate font-black text-[#071b3a]">{topic.topic_title}</p><p className="mt-1 text-xs text-slate-500">{questions} questions • {correct} correct</p></div><span className="shrink-0 rounded-full bg-violet-50 px-3 py-1 text-xs font-black text-violet-700">{mastery}%</span></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-gradient-to-r from-violet-600 to-cyan-400" style={{width:`${mastery}%`}}/></div></div>})}</div> : <p className="mt-6 rounded-2xl bg-slate-50 p-4 text-sm leading-6 text-slate-500">{empty}</p>}</section>}
