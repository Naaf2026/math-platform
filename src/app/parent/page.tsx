...
import { ArrowLeft, BarChart3, CheckCircle2, Flame, Gem, GraduationCap, Medal, ShieldCheck, Sparkles, Target, Trophy } from "lucide-react";
...
export default function ParentPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data, error } = await s.rpc("get_learning_report", { p_student_id: studentId });
      if (error || !data?.[0]) { setStatus(error?.message || "The linked student report is unavailable."); return; }
      setReport(data[0] as Report);
      const { data: tp } = await s.rpc("get_linked_student_topics", { p_student_id: studentId });
...
function MiniStat({ label, value }: { label: string; value: number }) { return <div className="rounded-2xl bg-slate-50 p-4"><p className="text-xs font-bold text-slate-500">{label}</p><p className="mt-1 text-xl font-black text-[#071b3a]">{value}</p></div>; }
