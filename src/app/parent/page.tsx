...
import { ArrowLeft, BarChart3, CalendarDays, CheckCircle2, Flame, Gem, GraduationCap, Lightbulb, Medal, ShieldCheck, Target, TrendingUp, Trophy } from "lucide-react";
...
export default function ParentPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const studentId = requestedLearnerId && linkedIds.includes(requestedLearnerId) ? requestedLearnerId : linkedIds[0];
      if (!studentId) { setStatus("No linked student found. A parent or guardian account must be explicitly linked to a student first."); return; }
      ...
      if (reportResult.error || !reportResult.data?.[0]) { setStatus(reportResult.error?.message || "The linked student report is unavailable."); return; }
      setReport(reportResult.data[0] as Report);
      setTopics((topicsResult.data ?? []) as Topic[]);
      if (weeklyResult.error) setWeeklyAvailable(false); else setWeekly((weeklyResult.data ?? []) as WeeklyDay[]);
      setStatus("");
    })();
  }, [requestedLearnerId]);

  const strongest = useMemo(() => [...topics].sort((a, b) => Number(b.mastery) - Number(a.mastery)).slice(0, 3), [topics]);
  const support = useMemo(() => [...topics].sort((a, b) => Number(a.mastery) - Number(b.mastery)).slice(0, 3), [topics]);
