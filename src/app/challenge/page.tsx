...
import { ArrowLeft, ArrowRight, Check, ChevronLeft, ChevronRight, LockKeyhole, MessageCircle, Trophy } from "lucide-react";
...
export default function Challenge() {
  ...
  async function load() {
    ...
    setQuestions(data.map((item: Q) => ({ ...item, options: Array.isArray(item.options) ? item.options : [] })));
    setXp(profile?.xp ?? 0); setStreak(profile?.current_streak ?? 0); setBest(profile?.best_streak ?? 0);
    setLoading(false);
  }
  ...
  async function submitAnswer(answer = draft) {
    ...
    if (userId) {
      ...
      if (profile) { setXp(profile.xp ?? 0); setStreak(profile.current_streak ?? 0); setBest(profile.best_streak ?? 0); }
    }
    setSaving(false);
  }
  ...
  function restart() {
    setIndex(0); setDraft(""); setSubmitted(false); setResultCorrect(null); setShowAnswer(false);
    ...
    <main className="min-h-screen bg-[#d8eef2] text-slate-800">
      <header className="sticky top-0 z-50 bg-[#13b7d2] text-white shadow-md">
        <div className="mx-auto flex h-16 max-w-[1500px] items-center justify-between px-4 sm:px-7">
          ...
          <div className="flex items-center gap-2"><span className="rounded-full bg-white/15 px-3 py-1.5 text-xs font-black">⚡ {xp} XP</span><span className="rounded-full bg-white/15 px-3 py-1.5 text-xs font-black">🔥 {streak}</span><span className="hidden rounded-full bg-white/20 px-3 py-1.5 text-xs font-black sm:inline">{stages[stage].name}</span></div>
        </div>
      </header>

      <div className="mx-auto flex max-w-[1500px]">
        <aside className="hidden w-[250px] shrink-0 border-r border-slate-300 bg-white md:block">
          <div className="border-b border-slate-200 bg-slate-50 px-5 py-4"><div className="text-xs font-black text-slate-500">PROFICIENCY</div><div className="mt-1 flex items-center justify-between"><span className="font-black">{stages[stage].name}</span><span className="text-sm font-black text-slate-500">{completed.length} / {questions.length}</span></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-slate-200"><div className="h-full rounded-full bg-cyan-500" style={{width:`${questions.length ? completed.length / questions.length * 100 : 0}%`}}/></div></div>
          <div className="py-2">
            {questions.map((question, n) => {
              const isCurrent = n === index;
              const isDone = completed.includes(question.id);
              return <button key={question.id} onClick={() => resetQuestion(n)} className={`flex w-full items-center gap-3 border-b border-slate-100 px-4 py-3 text-left transition ${isCurrent ? "bg-cyan-50" : "hover:bg-slate-50"}`}><span className={`grid h-8 w-8 shrink-0 place-items-center rounded-full border-2 text-sm font-black ${isDone ? "border-emerald-400 bg-emerald-400 text-white" : isCurrent ? "border-cyan-500 bg-cyan-500 text-white" : "border-cyan-300 bg-white text-cyan-600"}`}>{isDone ? <Check size={17}/> : n + 1}</span><span className="min-w-0"><span className="block text-sm font-black">Question {n + 1}</span><span className="block truncate text-[11px] text-slate-400">{question.skill || question.difficulty}</span><span className="text-[11px] font-bold text-slate-400">⚡ {n < 5 ? 1 : n < 8 ? 2 : 3}</span></span></button>;
            })}
          </div>
        </aside>

        <section className="min-w-0 flex-1 bg-[#247f96] px-3 py-4 sm:px-7 sm:py-7">
          <div className="mx-auto max-w-[980px]">
            <div className="mb-4 flex items-center justify-between text-white"><Link href="/challenge" className="inline-flex items-center gap-1 rounded-full bg-white/10 px-3 py-2 text-sm font-black"><ChevronLeft size={16}/>Challenge</Link><div className="text-sm font-black opacity-80">{index + 1} / {questions.length}</div></div>

            <div className="rounded-[2rem] border-[7px] border-yellow-400 bg-[#f8ffff] shadow-[0_15px_0_rgba(11,60,72,.25)] sm:rounded-[2.5rem] sm:border-[9px]">
              <div className="relative min-h-[590px] overflow-hidden rounded-[1.4rem] bg-white sm:rounded-[1.8rem]">
                <div className="border-b border-slate-200 bg-gradient-to-b from-[#f3ffff] to-white px-5 pb-4 pt-4 sm:px-10">
                  <div className="flex items-start justify-between gap-4"><div><div className="flex items-center gap-3"><span className="text-2xl font-black text-slate-700">Question</span><span className="grid h-9 w-9 place-items-center rounded-full bg-lime-400 font-black text-white">{index + 1}</span></div><div className="mt-1 flex items-center gap-2 text-xs font-black text-cyan-600"><span>{current.skill || "Mathematics"}</span><span className="text-slate-300">•</span><span>{current.difficulty || "Elementary"}</span></div></div><button onClick={changeQuestion} disabled={saving} className="rounded-full border border-cyan-200 bg-cyan-50 px-4 py-2 text-xs font-black text-cyan-700 hover:bg-cyan-100 disabled:opacity-40">Change Question</button></div>
                  <div className="mt-4 flex gap-1"><span className={`h-2 w-2 rounded-full ${submitted ? "bg-emerald-400" : "bg-orange-400"}`}/><span className={`h-2 w-2 rounded-full ${submitted ? "bg-emerald-400" : "bg-slate-200"}`}/><span className={`h-2 w-2 rounded-full ${submitted ? "bg-emerald-400" : "bg-slate-200"}`}/><span className="ml-2 text-[10px] font-black text-slate-400">{submitted ? "ANSWERED" : "YOUR TURN"}</span></div>
                </div>

                <div className="min-h-[330px] px-6 py-8 sm:px-12 sm:py-10">
                  <h2 className="max-w-4xl text-2xl font-bold leading-relaxed text-slate-800 sm:text-[27px]">{current.prompt}</h2>

                  {isManipulative && each > 0 && groups > 0 ? (
                    <div className="mt-8 rounded-3xl bg-slate-50 p-5"><div className="flex justify-between font-black"><span>Build the groups</span><span>{built} / {groups}</span></div><div className="mt-4 grid gap-3 sm:grid-cols-3">{Array.from({length:groups}).map((_, g)=><div key={g} className={`rounded-2xl border-2 bg-white p-3 ${g < built ? "border-cyan-400" : "border-dashed border-slate-200"}`}><div className="text-[10px] font-black text-slate-400">GROUP {g+1}</div><div className="mt-2 flex flex-wrap gap-1">{Array.from({length:each}).map((_, o)=><span key={o} className={`grid h-7 w-7 place-items-center rounded-lg text-xs ${g < built ? "bg-cyan-500 text-white" : "bg-slate-100 text-slate-300"}`}>●</span>)}</div></div>)}</div><button onClick={buildGroup} disabled={submitted || saving || built >= groups} className="mt-5 rounded-xl bg-cyan-600 px-6 py-3 font-black text-white disabled:opacity-40">Build Group</button></div>
                  ) : current.options.length > 0 ? (
                    <div className="mt-8 grid max-w-3xl gap-3 sm:grid-cols-2">{current.options.map((option, n)=><button key={option} onClick={()=>setDraft(option)} disabled={submitted || saving} className={`min-h-14 rounded-xl border-2 px-4 py-3 text-left font-bold transition ${draft === option ? "border-cyan-500 bg-cyan-50 text-cyan-800" : "border-slate-200 bg-white hover:border-cyan-300"} ${submitted && option === current.answer ? "border-emerald-400 bg-emerald-50" : ""}`}><span className="mr-3 inline-grid h-8 w-8 place-items-center rounded-lg bg-slate-100 text-sm font-black">{String.fromCharCode(65+n)}</span>{option}</button>)}</div>
                  ) : (
                    <div className="mt-8 max-w-xl"><label className="mb-2 block text-sm font-black text-slate-500">Your answer</label><input value={draft} onChange={(event)=>setDraft(event.target.value)} onKeyDown={(event)=>{if(event.key === "Enter") void submitAnswer();}} disabled={submitted || saving} placeholder="Type your answer here" className="h-14 w-full rounded-xl border-2 border-slate-300 px-4 text-lg outline-none focus:border-cyan-500 disabled:bg-slate-50"/></div>
                  )}

                  {error && <div className="mt-5 rounded-xl bg-rose-50 p-4 text-sm font-bold text-rose-700">{error}</div>}
                  {submitted && <div className={`mt-6 max-w-3xl rounded-2xl p-4 ${resultCorrect ? "bg-emerald-50 text-emerald-800" : "bg-amber-50 text-amber-800"}`}><div className="font-black text-lg">{resultCorrect ? "✓ Correct! Great job!" : "Not quite — keep going!"}</div><p className="mt-1 text-sm font-medium">{current.explanation}</p></div>}
                  {showAnswer && !submitted && <div className="mt-5 max-w-3xl rounded-2xl bg-blue-50 p-4 text-sm font-bold text-blue-800">Answer: <span className="font-black">{current.answer}</span></div>}
                </div>

                <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 bg-[#f6ffff] px-5 py-4 sm:px-8">
                  <div className="flex gap-2"><button onClick={()=>setShowAnswer((value)=>!value)} disabled={submitted} className="rounded-full border border-cyan-200 bg-white px-4 py-2 text-xs font-black text-cyan-700 disabled:opacity-40">{showAnswer ? "Hide Answer" : "Show Answer"}</button><button className="inline-flex items-center gap-1 rounded-full border border-slate-200 bg-white px-4 py-2 text-xs font-black text-slate-500"><MessageCircle size={14}/>Feedback</button></div>
                  <div className="flex gap-2">{!submitted && <button onClick={()=>void submitAnswer()} disabled={!draft.trim() || saving} className="rounded-full bg-orange-500 px-6 py-3 font-black text-white shadow-sm hover:bg-orange-600 disabled:opacity-40">{saving ? "Saving..." : "Submit Answer"}</button>}{submitted && <button onClick={nextQuestion} className="inline-flex items-center gap-2 rounded-full bg-cyan-600 px-6 py-3 font-black text-white">{index === questions.length - 1 ? "Finish" : "Next Question"}<ArrowRight size={17}/></button>}</div>
                </div>
              </div>
            </div>

            <div className="mt-4 flex items-center justify-between text-white"><button onClick={()=>index>0&&resetQuestion(index-1)} disabled={index===0} className="inline-flex items-center gap-1 rounded-xl bg-white/10 px-3 py-2 text-xs font-black disabled:opacity-30"><ChevronLeft size={15}/>Previous</button><div className="text-xs font-black opacity-80">⭐ {earned} XP earned today</div><button onClick={()=>index<questions.length-1&&resetQuestion(index+1)} disabled={index===questions.length-1} className="inline-flex items-center gap-1 rounded-xl bg-white/10 px-3 py-2 text-xs font-black disabled:opacity-30">Next<ChevronRight size={15}/></button></div>
          </div>
        </section>
      </div>
...
function Stat({ label, value }: { label: string; value: string }) {
  return <div className="rounded-2xl bg-slate-50 p-4"><div className="text-2xl font-black">{value}</div><div className="text-xs font-black uppercase tracking-wider text-slate-400">{label}</div></div>;
}
