...
export default function Challenge() {
  ...
  return (
    <main className="relative min-h-screen bg-[#d8eef2] text-slate-800">{celebration&&<div className="pointer-events-none fixed inset-0 z-[100] flex items-center justify-center overflow-hidden bg-cyan-950/15 backdrop-blur-[2px]">
        <div className="absolute inset-0" aria-hidden="true">{["★","✦","●","◆","★","✦","●","◆","★","✦","●","◆","★","✦","●","◆","★","✦"].map((symbol,i)=><span key={i} className={`absolute font-black ${i%3===0?"animate-ping":"animate-bounce"} ${i%4===0?"text-yellow-400":i%4===1?"text-cyan-400":i%4===2?"text-orange-400":"text-emerald-400"}`} style={{left:`${4+(i*11)%92}%`,top:`${5+(i*19)%86}%`,fontSize:`${18+(i%4)*8}px`,animationDelay:`${(i%7)*55}ms`,animationDuration:`${600+(i%5)*100}ms`}}>{symbol}</span>)}</div>
        <div className="relative mx-5 w-full max-w-sm">
          <div className="absolute -inset-8 animate-ping rounded-full bg-yellow-300/25"/>
          <div className="relative overflow-hidden rounded-[2.5rem] border-[5px] border-white bg-gradient-to-br from-yellow-200 via-white to-cyan-100 px-6 py-8 text-center shadow-[0_24px_80px_rgba(8,61,120,.35)]">
            <div className="absolute -left-8 -top-8 h-24 w-24 rounded-full bg-yellow-300/40"/><div className="absolute -bottom-10 -right-8 h-28 w-28 rounded-full bg-cyan-300/40"/>
            <div className="relative mx-auto grid h-24 w-24 animate-bounce place-items-center rounded-full bg-gradient-to-br from-yellow-300 to-orange-400 text-6xl shadow-xl ring-8 ring-white">★</div>
            <div className="relative mt-5 text-4xl font-black tracking-tight text-[#083d78]">{celebration}</div>
            <div className="relative mt-2 inline-flex items-center gap-2 rounded-full bg-emerald-500 px-5 py-2 text-base font-black text-white shadow-lg"><Check size={20} strokeWidth={4}/> Correct!</div>
            <div className="relative mt-5 text-sm font-black uppercase tracking-[.16em] text-cyan-700">⚡ Keep the streak going!</div>
          </div>
        </div>
      </div>}
