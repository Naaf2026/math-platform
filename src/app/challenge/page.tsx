...
export default function Challenge() {
  ...
  function startChallenge(selectedStage: ChallengeStage) {
    ...
    setQuestions(chosen);
    setIndex(0); setDraft(""); setSubmitted(false); setResultCorrect(null); setBuilt(0); setOrderedIds([]);
  ...
  function resetQuestion(nextIndex: number) {
    setIndex(nextIndex);
    setDraft(""); setSubmitted(false); setResultCorrect(null); setBuilt(0); setOrderedIds([]); setError("");
  }
  ...
  function initializeOrder() {
    ...
    if (orderedIds.length !== ids.length || ids.some((id) => !orderedIds.includes(id))) {
  ...
  function orderingCanonicalAnswer(): string {
    ...
    const numeric = current.options.map((option) => ({
      id: option.id,
      value: Number(String(option.text).replace(/,/g, "").trim()),
    }));
  ...
  async function submitAnswer(answer = draft) {
    ...
    const normalizedAnswer = isOrdering ? orderedIds.join(",") : answer.trim();
    ...
    if (isOrdering) {
      // Ordering questions are validated from their displayed choices so malformed
      // generated answer strings cannot mark a correctly arranged sequence wrong.
      const canonical = orderingCanonicalAnswer();
      const isCorrectLocally = normalizedAnswer === canonical;
      const { data: saved, error: rpcError } = await supabase.rpc("submit_learning_answer", {
      ...
      // The RPC remains the source of XP/history. If its legacy answer comparison
      // disagrees with a valid ordering derived from the displayed numbers, use the
      // deterministic ordering result for this question's UI result.
      if (!submitError && data?.[0] && isCorrectLocally) {
        data[0] = { ...data[0], is_correct: true };
      }
    } else {
      const result = await supabase.rpc("submit_learning_answer", {
        p_question_id: current.id,
        p_selected_answer: normalizedAnswer,
      });
      data = result.data;
    ...
    setDraft(normalizedAnswer); setResultCorrect(isCorrect); setSubmitted(true);
  ...
  function restart() {
    ...
    setCorrect(0); setEarned(0); setCompleted([]); setBuilt(0); setOrderedIds([]); setDone(false); setError("");
  }
  ...
  const correctAnswerText = isOrdering ? orderingAnswerText(orderingCanonicalAnswer().split(",").filter(Boolean)) : (correctOption?.text || current.answer);
