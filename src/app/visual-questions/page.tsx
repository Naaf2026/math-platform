...
export default function VisualQuestionsPage(){
 ...
 const [error,setError]=useState(""); const [earned,setEarned]=useState(0); const [rewardToast,setRewardToast]=useState<{sparks:number;coins:number}|null>(null);
 ...
 async function answer(value:string){
  ...
  if(ok){
   ...
   if(supabase){
    ...
    window.setTimeout(()=>setRewardToast(null),1100);
 ...
 return <main className="relative h-[100dvh] overflow-hidden bg-[#eef9ff] text-[#083d78]">{rewardToast&&<div className="pointer-events-none fixed left-1/2 top-20 z-[100] -translate-x-1/2 rounded-2xl bg-white px-5 py-3 text-center font-black text-[#083d78] shadow-xl ring-1 ring-sky-100">✨ +{rewardToast.sparks} Mind Sparks&nbsp;&nbsp; 🪙 +{rewardToast.coins} MC</div>}
...
function FitToCard({children,watchKey}:{children:React.ReactNode;watchKey:string}){
 ...
 return <div ref={frame} className="h-full w-full overflow-y-auto overscroll-contain pb-3 md:overflow-hidden"><div ref={content} style={{transform:`scale(${scale})`,transformOrigin:"top left",width:`${100/scale}%`}}>{children}</div></div>;
}
