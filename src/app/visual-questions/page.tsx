...
export default function VisualQuestionsPage(){
 ...
 return <main className="min-h-screen bg-[#eef9ff] pb-24 text-[#083d78] md:pb-0">
  ...
  <div className="mx-auto flex min-h-[calc(100dvh-73px)] max-w-6xl flex-col px-4 py-2.5 sm:px-6 sm:py-3"><section className="rounded-2xl bg-gradient-to-r from-[#197fe9] via-[#2877e8] to-[#5367ea] px-4 py-1.5 text-white shadow-md sm:px-5 sm:py-2"><div className="flex items-center gap-3"><div className="inline-flex shrink-0 items-center gap-2 rounded-full bg-white/15 px-2.5 py-1 text-[10px] font-black uppercase tracking-wider"><Eye size={14}/> Visual Questions</div><div className="hidden h-5 w-px bg-white/35 sm:block"/><p className="min-w-0 flex-1 truncate text-sm font-bold text-blue-50">Question {index+1} of {questions.length} • {q.skill||"Visual Maths"}</p><div className="shrink-0 rounded-xl bg-white/10 px-3 py-1 text-center"><p className="text-base font-black leading-none">{remaining===null?"∞":remaining}</p><p className="text-[8px] font-black uppercase text-white/75">Remaining</p></div></div><div className="mt-1 h-1 overflow-hidden rounded-full bg-white/15"><div className="h-full rounded-full bg-yellow-300 transition-all" style={{width:`${Math.max(5,progress)}%`}}/></div></section>
   <section className="mt-2 flex min-h-0 flex-1 flex-col rounded-[2rem] bg-white shadow-xl ring-1 ring-slate-100"><div className="border-b border-slate-100 px-5 py-2.5 sm:px-8"><div className="flex flex-wrap items-center justify-between gap-2"><span className="rounded-full bg-sky-50 px-3 py-1.5 text-xs font-black text-sky-700">{q.skill||"Visual Question"}</span><span className="rounded-full bg-slate-100 px-3 py-1.5 text-xs font-black text-slate-600">{q.difficulty}</span><span className="rounded-full bg-yellow-50 px-3 py-1.5 text-xs font-black text-orange-700">⭐ +{q.points??10} XP</span></div></div><div className="flex min-h-0 flex-1 flex-col px-5 py-3 sm:px-8 sm:py-4 lg:px-10"><h2 className="text-[clamp(1.35rem,2.4vw,2rem)] font-black leading-tight text-[#15233f]">{q.prompt}</h2><InteractiveQuestionEngine key={"".concat(q.id,"-",retryVersion)} question={q as never} selected={selected} disabled={selected!==null} onAnswer={answer} onRetry={()=>{if(advanceTimer.current!==null){window.clearTimeout(advanceTimer.current);advanceTimer.current=null;}setSelected(null);setRetryVersion(v=>v+1);}}/></div></section></div>
  <LearnerBottomNav/>
 </main>;
}
