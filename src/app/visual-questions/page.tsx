...
export default function VisualQuestionsPage(){
 ...
 return <main className="min-h-screen bg-[#eef9ff] pb-24 text-[#083d78] md:pb-0">
  <header className="sticky top-0 z-30 border-b border-[#dcecf6] bg-white/95 backdrop-blur"><div className="mx-auto flex max-w-7xl items-center gap-3 px-4 py-3 sm:px-6"><Link href="/dashboard" className="grid h-10 w-10 shrink-0 place-items-center rounded-2xl bg-[#eef6fc] text-[#197fe9]"><ArrowLeft size={18}/></Link><div className="min-w-0 md:w-[245px] md:shrink-0 lg:w-[280px]"><p className="text-[10px] font-black uppercase tracking-[.2em] text-[#197fe9]">Premium • Visual Maths</p><h1 className="truncate text-base font-black sm:text-lg">See it. Think it. Solve it. 👀</h1></div><nav className="hidden min-w-0 flex-1 items-center justify-center gap-1 xl:gap-3 md:flex" aria-label="Student navigation desktop"><Link href="/dashboard" className="flex items-center gap-2 rounded-xl px-2 py-2 text-sm font-black hover:bg-sky-50 xl:px-2.5 xl:text-base"><Home size={21}/>Home</Link><Link href="/brain-games" className="flex items-center gap-2 rounded-xl px-2 py-2 text-sm font-black hover:bg-sky-50 xl:px-2.5 xl:text-base"><Gamepad2 size={21}/>Games</Link><Link href="/leaderboard" className="flex items-center gap-2 rounded-xl px-2 py-2 text-sm font-black hover:bg-sky-50 xl:px-2.5 xl:text-base"><Trophy size={21}/>Leaderboard</Link><Link href="/rewards" className="flex items-center gap-2 rounded-xl px-2 py-2 text-sm font-black hover:bg-sky-50 xl:px-2.5 xl:text-base"><Gift size={21}/>Rewards</Link><Link href="/progress" className="flex items-center gap-2 rounded-xl px-2 py-2 text-sm font-black hover:bg-sky-50 xl:px-2.5 xl:text-base"><BarChart3 size={21}/>Progress</Link><Link href="/profile" className="flex items-center gap-2 rounded-xl px-2 py-2 text-sm font-black hover:bg-sky-50 xl:px-2.5 xl:text-base"><GraduationCap size={21}/>Profile</Link></nav><div className="ml-auto shrink-0 rounded-2xl bg-[#fff4cf] px-3 py-2 text-xs font-black text-[#806a12]"><Crown size={14} className="mr-1 inline"/> {access.plan_name}</div></div></header>
  <div className="mx-auto flex min-h-[calc(100vh-73px)] max-w-6xl flex-col px-4 py-4 sm:px-6 sm:py-5"><section className="rounded-[1.6rem] bg-gradient-to-r from-[#197fe9] via-[#2877e8] to-[#5367ea] px-5 py-3.5 text-white shadow-lg sm:px-6 sm:py-4"><div className="flex items-center gap-3"><div className="inline-flex shrink-0 items-center gap-2 rounded-full bg-white/15 px-3 py-1.5 text-[10px] font-black uppercase tracking-wider"><Eye size={14}/> Visual Questions</div><div className="hidden h-5 w-px bg-white/35 sm:block"/><p className="min-w-0 flex-1 truncate text-sm font-bold text-blue-50">Question {index+1} of {questions.length} • {q.skill||"Visual Maths"}</p><div className="shrink-0 rounded-2xl bg-white/10 px-4 py-2 text-center"><p className="text-xl font-black leading-none">{remaining===null?"∞":remaining}</p><p className="mt-1 text-[9px] font-black uppercase text-white/75">Remaining</p></div></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-white/15"><div className="h-full rounded-full bg-yellow-300 transition-all" style={{width:`${Math.max(5,progress)}%`}}/></div></section>
   <section className="mt-4 flex flex-1 flex-col overflow-hidden rounded-[2rem] bg-white shadow-xl ring-1 ring-slate-100"><div className="border-b border-slate-100 px-5 py-4 sm:px-8"><div className="flex flex-wrap items-center justify-between gap-2"><span className="rounded-full bg-sky-50 px-3 py-1.5 text-xs font-black text-sky-700">{q.skill||"Visual Question"}</span><span className="rounded-full bg-slate-100 px-3 py-1.5 text-xs font-black text-slate-600">{q.difficulty}</span><span className="rounded-full bg-yellow-50 px-3 py-1.5 text-xs font-black text-orange-700">⭐ +{q.points??10} XP</span></div></div><div className="flex flex-1 flex-col px-5 py-6 sm:px-10 sm:py-8"><h2 className="text-2xl font-black leading-snug text-[#15233f] sm:text-3xl">{q.prompt}</h2><InteractiveQuestionEngine key={"".concat(q.id,"-",retryVersion)} question={q as never} selected={selected} disabled={selected!==null} onAnswer={answer} onRetry={()=>{if(advanceTimer.current!==null){window.clearTimeout(advanceTimer.current);advanceTimer.current=null;}setSelected(null);setRetryVersion(v=>v+1);}}/></div></section></div>
  <LearnerBottomNav/>
 </main>;
}
