...
const content:Record<string,LessonContent> = {
  "g1-counting-adding":{idea:"Count objects carefully, then put two groups together to add.",steps:["Touch or point to each object as you count it once.","Count the first group and the second group.","Start with the first number and count on for the second group."],example:"You have 4 shells and find 3 more: 4, then count on 5, 6, 7. There are 7 shells.",explanation:"4 + 3 = 7. Drawing four dots and three dots shows the same total.",remember:"The last number you say is how many objects there are."},
  "g1-number-names":{idea:"Every number has a numeral and a name.",steps:["Look at the tens digit first.","Say how many tens there are.","Read the ones digit to finish the number name."],example:"24 is twenty-four: two tens and four ones.",explanation:"The 2 in 24 means twenty, and the 4 means four.",remember:"For 11 to 19, practise the special names: eleven, twelve, thirteen and so on."},
  "g1-tens-ones":{idea:"A group of ten ones is called one ten.",steps:["Make groups of ten objects.","Count the full groups of ten.","Count the leftover ones."],example:"23 is 2 tens and 3 ones: 10 + 10 + 3.",explanation:"Two groups of ten make 20, then three more make 23.",remember:"A ten is ten objects together, not just one object."},
  "g1-before-after":{idea:"Numbers have neighbours: one before and one after.",steps:["Find the number on a number line or count aloud.","Count back once to find the number before.","Count forward once to find the number after."],example:"Before 18 is 17; after 18 is 19.",explanation:"17, 18, 19 are consecutive numbers.",remember:"Before means one less; after means one more."},
  "g1-comparing":{idea:"Compare numbers to see which is greater, smaller or equal.",steps:["For two-digit numbers, compare tens first.","If the tens are equal, compare the ones.","Say greater than, less than or equal to."],example:"42 is greater than 37 because 4 tens is more than 3 tens.",explanation:"Four tens already make 40, which is greater than 37.",remember:"If both numbers have the same tens, look at the ones."},
  "g1-ordering":{idea:"Put numbers in order by comparing their values.",steps:["Choose whether to start with the smallest or the greatest.","Compare tens, then ones.","Place each number in its correct position."],example:"In order from smallest to greatest: 18, 23, 31.",explanation:"One ten is smaller than two tens, and two tens is smaller than three tens.",remember:"Smallest to greatest means the numbers get bigger as you go."},
  "g1-patterns":{idea:"A number pattern follows a rule that repeats.",steps:["Look at the difference between neighbouring numbers.","Check that the same rule works again.","Use the rule to find the next number."],example:"12, 14, 16, 18, … The next number is 20.",explanation:"We add 2 each time.",remember:"Check the rule with more than one pair of numbers."},
  "g1-skip-counting":{idea:"Skip counting counts by the same amount each time.",steps:["Choose a step, such as 2, 5 or 10.","Add the step to say the next number.","Keep using the same step."],example:"Count by fives: 5, 10, 15, 20, 25.",explanation:"Each number is five more than the number before it.",remember:"Counting by tens changes the tens while the ones digit stays the same."},
...
export default function LessonPage(){
  ...
  return <main className="min-h-screen bg-[#f7f9fc]">
    ...
    <div className="mx-auto max-w-3xl px-5 py-8">
      {done?<section className="rounded-3xl bg-white p-8 text-center shadow-sm"><CheckCircle2 className="mx-auto text-emerald-600" size={42}/><h1 className="mt-4 text-3xl font-black text-[#071b3a]">Lesson complete</h1><p className="mt-2 text-slate-600">{lesson.title}</p><div className="mt-6 flex flex-wrap justify-center gap-3"><Link href={`/learn/${topicId}`} className="inline-flex items-center gap-2 rounded-xl bg-[#071b3a] px-5 py-3 font-bold text-white">More lessons <ArrowRight size={17}/></Link>{topic.level==="Grade 1"&&<Link href={`/training/practice?topic=${encodeURIComponent(topicId)}&skill=${encodeURIComponent(lesson.title)}&grade=Grade%201`} className="inline-flex items-center gap-2 rounded-xl border border-[#0d666b] px-5 py-3 font-bold text-[#0d666b]">Practise this skill <ArrowRight size={17}/></Link>}</div></section>:
