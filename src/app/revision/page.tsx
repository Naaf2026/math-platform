...
export default function RevisionPage() {
  ...
  async function openGiftBag(){
    if(giftLoading||giftOpen)return;
    setGiftLoading(true);
    const supabase=createClient();
    if(!supabase){setGiftLoading(false);return;}
    const {data,error}=await supabase.rpc("claim_revision_gift_bag",{p_score:correctCount});
    if(error){setGiftMessage(error.message);setGiftLoading(false);return;}
    const result=data as {reward?:number;already_claimed?:boolean;balance?:number}|null;
    setGiftReward(result?.reward??0);
    setGiftMessage(result?.already_claimed?"Today's Mind Sparks Gift Bag was already opened.":`You earned ${result?.reward??0} Mind Sparks! ✨`);
    setGiftOpen(true);
    ...
    <main className="min-h-screen bg-[#3f4f91] text-[#16234e]">
      ...
      <section className="mx-auto max-w-[1120px] px-3 py-6 sm:px-6">
        <div className="relative">
          ...
          <div className="overflow-hidden rounded-[34px] border-[10px] border-[#f4d940] bg-[#eaf8fb] shadow-2xl">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#d7edf2] px-5 py-5 sm:px-10">
              <div>
                <span className="text-3xl font-black text-[#ff6900]">Question {index+1}</span>
                <span className="ml-3 text-sm font-bold text-slate-400">{current.skill||current.topic||grade}</span>
              </div>
              <div className="flex flex-wrap gap-2">
                <button disabled={index===0} onClick={()=>setIndex(i=>Math.max(0,i-1))} className="inline-flex items-center gap-1 rounded-full border border-[#8fd7e7] bg-white px-4 py-2 font-bold text-[#46b9d4] disabled:opacity-40"><ChevronLeft size={18}/> Previous Qn</button>
                <button disabled={index===questions.length-1||!value.trim()} onClick={()=>setIndex(i=>Math.min(questions.length-1,i+1))} className="inline-flex items-center gap-1 rounded-full border border-[#8fd7e7] bg-white px-4 py-2 font-bold text-[#46b9d4] disabled:opacity-40">Next <ChevronRight size={18}/></button>
                <button onClick={submitRevision} disabled={!allAnswered||submitting} className="rounded-full bg-[#3655ba] px-5 py-2 font-black text-white disabled:opacity-40">{submitting?"Submitting…":"Submit"}</button>
              </div>
            </div>
              ...
              <div className="max-w-[850px]">
                ...
                <div className="mt-8">
                  {(current.question_type==="multiple_choice"||current.question_type==="true_false") ? (
                    ...
                    <div>
                      <p className="mb-3 text-lg font-medium text-slate-400">Write your answer</p>
                      <input value={value} onChange={e=>setAnswers(p=>({...p,[current.id]:e.target.value}))} inputMode={current.question_type==="number_input"?"numeric":"text"} className="h-14 w-full max-w-xs rounded-xl border-2 border-slate-300 bg-white px-4 text-2xl font-bold outline-none focus:border-[#3655ba]"/>
                    </div>
                  )}
                </div>
              </div>
        ...
        <div className="mt-5 flex justify-center gap-2">
          {questions.map((q,i)=><button key={q.id} onClick={()=>setIndex(i)} className={`h-3 rounded-full transition-all ${i===index?"w-10 bg-yellow-300":"w-3 bg-white/50"}`} aria-label={`Question ${i+1}`}/>)}
        </div>
      </section>
    </main>
  );
