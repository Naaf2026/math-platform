...
type SubscriptionStatus = "premium" | "trial" | "free" | "expired" | "not_applicable";
type UserRow = {
  id: string; full_name: string | null; display_name: string | null; grade: string | null; learning_level: string | null;
  role: string; account_status: AccountStatus; subscription_status: SubscriptionStatus; subscription_plan: string | null; subscription_ends_at: string | null;
};
...
function SubscriptionBadge({ status, endsAt }: { status: SubscriptionStatus; endsAt: string | null }) {
  if (status === "not_applicable") return <span className="text-xs font-bold text-slate-300">Not a learner</span>;
  const labels: Record<SubscriptionStatus, string> = { premium: "Premium", trial: "3-Day Trial", free: "Free", expired: "Expired", not_applicable: "Not a learner" };
  const styles: Record<SubscriptionStatus, string> = {
    premium: "bg-emerald-100 text-emerald-800 ring-emerald-200", trial: "bg-amber-100 text-amber-800 ring-amber-200",
    free: "bg-slate-100 text-slate-600 ring-slate-200", expired: "bg-rose-100 text-rose-800 ring-rose-200", not_applicable: "bg-slate-50 text-slate-300 ring-slate-100",
  };
  const endLabel = endsAt && status !== "free" ? new Date(endsAt).toLocaleDateString(undefined, { day: "numeric", month: "short", year: "numeric" }) : "";
...
export default function AdminUsersPage() {
  ...
  const learnerCount = users.filter(u => u.role === "student").length;
  const premiumCount = users.filter(u => u.role === "student" && u.subscription_status === "premium").length;
  const trialCount = users.filter(u => u.role === "student" && u.subscription_status === "trial").length;
  const freeCount = users.filter(u => u.role === "student" && u.subscription_status === "free").length;
  const expiredCount = users.filter(u => u.role === "student" && u.subscription_status === "expired").length;
  ...
  return <main className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-violet-50 p-5 pb-24 sm:p-8"><div className="mx-auto max-w-7xl">
    ...
    <section className="mt-5 rounded-3xl bg-white p-5 shadow-sm ring-1 ring-slate-100 sm:p-6"><div className="flex flex-wrap items-center justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-wider text-violet-600">Learner access</p><h2 className="mt-1 text-xl font-black text-[#071b3a]">Trial & Premium overview</h2><p className="mt-1 text-sm text-slate-500">See which students are on the 3-day trial, Premium, Free, or have expired access.</p></div><div className="flex flex-wrap gap-2 text-xs font-black"><span className="rounded-full bg-slate-100 px-3 py-1.5 text-slate-600">{learnerCount} learners</span><span className="rounded-full bg-emerald-100 px-3 py-1.5 text-emerald-700">{premiumCount} Premium</span><span className="rounded-full bg-amber-100 px-3 py-1.5 text-amber-700">{trialCount} Trial</span><span className="rounded-full bg-slate-100 px-3 py-1.5 text-slate-600">{freeCount} Free</span><span className="rounded-full bg-rose-100 px-3 py-1.5 text-rose-700">{expiredCount} Expired</span></div></div></section>
