...
export default function SubscriptionBadge({ status, endsAt }: { status: SubscriptionStatus; endsAt: string | null }) {
  if (status === "not_applicable") return <span className="text-xs font-bold text-slate-300">Not a learner</span>;
  const labels: Record<SubscriptionStatus, string> = { premium: "Premium", trial: "7-Day Trial", free: "Free", expired: "Expired", not_applicable: "Not a learner" };
  const styles: Record<SubscriptionStatus, string> = {
    premium: "bg-emerald-100 text-emerald-800 ring-emerald-200",
    ...
    not_applicable: "bg-slate-50 text-slate-300 ring-slate-100",
  };
  ...
  return <main className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-violet-50 p-5 pb-24 sm:p-8"><div className="mx-auto max-w-7xl">
    ...
    <section className="mt-5 rounded-3xl bg-white p-5 shadow-sm ring-1 ring-slate-100 sm:p-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <p className="text-xs font-black uppercase tracking-wider text-violet-600">Learner access</p>
          <h2 className="mt-1 text-xl font-black text-[#071b3a]">Trial & Premium overview</h2>
          <p className="mt-1 text-sm text-slate-500">See which students are on the 7-day trial, Premium, Free, or have expired access.</p>
        </div>
        <div className="flex flex-wrap gap-2 text-xs font-black">
          <span className="rounded-full bg-slate-100 px-3 py-1.5 text-slate-600">{learnerCount} learners</span>
          <span className="rounded-full bg-emerald-100 px-3 py-1.5 text-emerald-700">{premiumCount} Premium</span>
          <span className="rounded-full bg-amber-100 px-3 py-1.5 text-amber-700">{trialCount} Trial</span>
          <span className="rounded-full bg-slate-100 px-3 py-1.5 text-slate-600">{freeCount} Free</span>
          <span className="rounded-full bg-rose-100 px-3 py-1.5 text-rose-700">{expiredCount} Expired</span>
        </div>
      </div>
    </section>
    ...
    <p className="mt-5 text-center text-xs text-slate-400">Subscription status is read from the learner's current subscription. Premium means an active Premium plan; Trial means the 7-day trial is still valid. Administrator accounts are protected. Account status is controlled by a server-side administrator function and can be changed again later.</p>
  </div></main>;
}
