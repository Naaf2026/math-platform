...
export default function AdminUsersPage() {
  ...
  async function changeRole(user: UserRow, role: AppRole) {
    if (!supabase || user.role === role) return;
    setBusyId(user.id);
    setNotice("");
  ...
  if (status) return <main className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-violet-50 p-6"><div className="mx-auto mt-24 max-w-xl rounded-[2rem] bg-white p-10 text-center shadow-xl ring-1 ring-slate-100"><ShieldCheck className="mx-auto text-violet-600" size={48}/><h1 className="mt-4 text-2xl font-black text-[#071b3a]">Admin User Management</h1><p className="mt-3 text-sm leading-6 text-slate-500">{status}</p><Link href="/admin/classes" className="mt-6 inline-flex items-center gap-2 rounded-2xl bg-violet-600 px-5 py-3 font-black text-white"><ArrowLeft size={17}/> Admin console</Link></div></main>;

  const counts = roleOptions.reduce<Record<string, number>>((acc, r) => { acc[r.value] = users.filter(u => u.role === r.value).length; return acc; }, {});
  ...
  return <main className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-violet-50 p-5 pb-24 sm:p-8"><div className="mx-auto max-w-7xl">
    <header className="flex flex-wrap items-center justify-between gap-4"><Link href="/admin/classes" className="inline-flex items-center gap-2 font-bold text-slate-600"><ArrowLeft size={18}/> Admin console</Link><button onClick={load} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-bold text-slate-600"><RefreshCw size={16}/> Refresh</button></header>
    <section className="mt-7 rounded-[2rem] bg-gradient-to-br from-[#17234f] via-violet-700 to-blue-600 p-7 text-white shadow-2xl sm:p-9"><div className="flex flex-wrap items-end justify-between gap-6"><div><div className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1.5 text-xs font-black uppercase tracking-wider"><ShieldCheck size={14}/> Secure administrator view</div><h1 className="mt-4 text-4xl font-black">User Management</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-indigo-100">Review platform users and assign approved public roles. Administrator promotion is intentionally excluded from this screen.</p></div><div className="rounded-2xl bg-white/10 p-4 text-center"><Users className="mx-auto text-indigo-100" size={25}/><div className="mt-1 text-2xl font-black">{users.length}</div><div className="text-xs font-bold text-indigo-100">Users shown</div></div></div></section>
    <section className="mt-6 grid grid-cols-2 gap-3 md:grid-cols-4">{roleOptions.map(r=><button key={r.value} onClick={()=>setFilter(filter===r.value?"all":r.value)} className={`rounded-2xl p-4 text-left shadow-sm ring-1 transition ${filter===r.value?"bg-violet-50 ring-violet-200":"bg-white ring-slate-100 hover:ring-violet-100"}`}><div className="text-xs font-black uppercase tracking-wider text-slate-500">{r.label}</div><div className="mt-1 text-2xl font-black text-[#071b3a]">{counts[r.value] ?? 0}</div></button>)}</section>
    ...
    <section className="mt-6 rounded-3xl bg-white p-5 shadow-sm ring-1 ring-slate-100 sm:p-7"><div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between"><div><p className="text-xs font-black uppercase tracking-wider text-violet-600">Directory</p><h2 className="text-2xl font-black text-[#071b3a]">Platform users</h2></div><div className="relative w-full md:max-w-sm"><Search className="absolute left-3 top-3 text-slate-400" size={18}/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search name, grade or role" className="w-full rounded-2xl border border-slate-200 py-3 pl-10 pr-4 text-sm outline-none focus:border-violet-500"/></div></div>
      <div className="mt-5 overflow-x-auto"><table className="w-full min-w-[720px] text-left"><thead><tr className="border-b border-slate-100 text-xs font-black uppercase tracking-wider text-slate-400"><th className="px-3 py-3">User</th><th className="px-3 py-3">Grade</th><th className="px-3 py-3">Level</th><th className="px-3 py-3">Role</th><th className="px-3 py-3">Change role</th></tr></thead><tbody>{filtered.length===0?<tr><td colSpan={5} className="px-3 py-10 text-center text-sm text-slate-500">No matching users.</td></tr>:filtered.map(u=><tr key={u.id} className="border-b border-slate-50 last:border-0"><td className="px-3 py-4"><div className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-violet-50 font-black text-violet-700"><UserCog size={18}/></div><div><div className="font-black text-[#071b3a]">{u.display_name||u.full_name||"Unnamed user"}</div><div className="max-w-[260px] truncate text-xs text-slate-400">{u.id}</div></div></div></td><td className="px-3 py-4 text-sm font-semibold text-slate-600">{u.grade||"—"}</td><td className="px-3 py-4 text-sm font-semibold text-slate-600">{u.learning_level||"—"}</td><td className="px-3 py-4"><span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-black capitalize text-slate-700">{u.role}</span></td><td className="px-3 py-4"><select disabled={busyId===u.id} value={u.role} onChange={e=>changeRole(u,e.target.value as AppRole)} className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-bold text-slate-700 outline-none focus:border-violet-500">{roleOptions.map(r=><option key={r.value} value={r.value}>{r.label}</option>)}</select></td></tr>)}</tbody></table></div></section>
    <p className="mt-5 text-center text-xs text-slate-400">Role changes affect access to the learning platform. Admin accounts can only be assigned through a separate administrator-controlled workflow.</p>
  </div></main>;
}
