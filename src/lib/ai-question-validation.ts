...
function check(key: string, label: string, severity: ValidationSeverity, message: string): ValidationCheck {
...
function answerConsistency(question: InteractiveQuestion): ValidationCheck {
  ...
  if (type !== "multiple_choice") {
    return check("answer", "Answer consistency", "pass", "Answer format is handled by the selected interaction type.");
  }
  const options = question.options || [];
  const matches = options.filter((option) => answersMatch(option, question.answer));
  if (matches.length !== 1) {
    return check("answer", "Answer consistency", "error", matches.length === 0 ? "The correct answer is not present in the options." : "More than one option matches the correct answer.");
  }
  return check("answer", "Answer consistency", "pass", "Exactly one option matches the correct answer.");
}
...
function interactionQuality(question: InteractiveQuestion): ValidationCheck {
  ...
  if (type === "drag_drop") {
    ...
    if (!Array.isArray(zones) || zones.length < 1) return check("interaction", "Interaction setup", "error", "Drag-and-drop questions need at least one zone.");
  }
  if (type === "number_line" && Number(config.max ?? 10) <= Number(config.min ?? 0)) {
    return check("interaction", "Interaction setup", "error", "Number-line maximum must be greater than minimum.");
  }
  return check("interaction", "Interaction setup", "pass", `Interaction type “${type.replaceAll("_", " ")}” has a valid basic configuration.`);
}

function explanationQuality(question: InteractiveQuestion): ValidationCheck {
  if (!question.explanation?.trim()) return check("explanation", "Explanation", "warning", "Add an explanation so learners can understand why the answer is correct.");
  return check("explanation", "Explanation", "pass", "Explanation is provided for learner feedback.");
}
...
export function validateQuestionForPublishing(question: QuestionBankItem | InteractiveQuestion, bank: QuestionBankItem[] = []): QuestionValidationReport {
  ...
  const checks: ValidationCheck[] = [
    ...structuralErrors,
    answerConsistency(question),
    wordingQuality(question),
    optionQuality(question),
    interactionQuality(question),
    explanationQuality(question),
  ];

  const duplicate = bank.find((item) => item.id !== ("id" in question ? question.id : "") && similarity(item.prompt, question.prompt) >= 0.82);
  if (duplicate) {
    checks.push(check("duplicate", "Duplicate detection", "warning", `Very similar question found: “${duplicate.title}”. Review before publishing.`));
  } else {
    checks.push(check("duplicate", "Duplicate detection", "pass", "No close duplicate was found in the current question bank."));
  }

  const errors = checks.filter((item) => item.severity === "error").length;
  const warnings = checks.filter((item) => item.severity === "warning").length;
  const score = Math.max(0, Math.min(100, 100 - errors * 25 - warnings * 7));
  const publishable = errors === 0;
  const summary = publishable
    ? warnings ? `Ready with ${warnings} review ${warnings === 1 ? "item" : "items"}.` : "All validation checks passed."
    : `${errors} blocking ${errors === 1 ? "issue" : "issues"} must be fixed before publishing.`;
