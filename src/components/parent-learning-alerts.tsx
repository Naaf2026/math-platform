    ...
    <section className="mb-6 rounded-3xl bg-white p-5 shadow-sm ring-1 ring-violet-100 sm:p-6" aria-label="Learning alerts">
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button type="button" onClick={() => setOpen(value => !value)} className="rounded-xl px-3 py-2 text-xs font-black text-slate-500 hover:bg-slate-50" aria-expanded={open}>{open ? "Hide" : "Show"}</button>
        </div>
      </div>
      ...
      {open && alerts.length > 0 && <div className="mt-5 space-y-3">{alerts.map(alert => {
        ...
        return <article key={alert.alert_id} className={`rounded-2xl border p-4 transition ${priorityClass(alert.priority)} ${alert.read_at ? "opacity-75" : ""}`}>
          <div className="flex gap-3"><span className="mt-0.5 shrink-0 rounded-xl bg-white/80 p-2"><Icon size={18}/></span><div className="min-w-0 flex-1">
            <div className="flex flex-wrap items-start justify-between gap-2"><div><h3 className="font-black">{alert.title}</h3>{!alert.read_at && <span className="mt-1 inline-flex rounded-full bg-white/80 px-2 py-0.5 text-[10px] font-black uppercase tracking-wider">New</span>}</div>{!alert.read_at && <button type="button" onClick={() => void markRead(alert.alert_id)} className="inline-flex items-center gap-1.5 rounded-xl bg-white px-3 py-2 text-xs font-black shadow-sm hover:bg-slate-50"><Check size={14}/> Mark as read</button>}</div>
            <p className="mt-2 text-sm leading-6">{alert.message}</p><p className="mt-2 text-[11px] font-semibold opacity-70">{new Date(alert.created_at).toLocaleDateString(undefined, { month: "short", day: "numeric" })}</p>
          </div></div>
        </article>;
      })}</div>}

      {open && !alerts.length && !error && <div className="mt-5 rounded-2xl bg-slate-50 p-5 text-center text-sm text-slate-500">No learning alerts right now. Keep encouraging steady practice!</div>}
    </section>
  );
}
