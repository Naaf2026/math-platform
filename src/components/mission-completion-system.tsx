...
export default function MissionCompletionSystem() {
  ...
  useEffect(() => {
    ...
    const onAnswer = (event: Event) => {
      ...
      setResult(prev => {
        ...
        return { score: prev.score + earned, correct: prev.correct + (correct ? 1 : 0), answered: prev.answered + 1, lives, combo, perfect: prev.perfect && correct && !d.hintUsed, timedOut: prev.timedOut };
      });
    };
    ...
    const onRestart = () => {
      ...
      setVisible(false); setCompleted(false);
    };
  ...
  useEffect(() => {
    ...
    if (result.answered >= TOTAL_CHALLENGES && !completed) {
      setCompleted(true);
      setVisible(true);
      window.dispatchEvent(new CustomEvent("fv:mission-complete", { detail: result }));
    }
  }, [result, completed]);
  ...
  return <div className="fixed inset-0 z-[95] flex items-center justify-center bg-slate-950/70 p-4 backdrop-blur-md">
    <div className="w-full max-w-xl overflow-hidden rounded-[2.25rem] bg-white shadow-2xl ring-1 ring-white/30">
      ...
      </div><div className="mt-5 rounded-2xl border border-slate-100 bg-slate-50 p-4 text-sm font-bold leading-6 text-slate-600">{outOfLives ? "All three lives were used. Your saved learning progress is kept. Start a fresh adventure when you're ready." : "Your final Mission score is calculated from the submitted answer results, combo multiplier, difficulty and hint usage."}</div>
      <button onClick={() => window.location.reload()} className="mt-5 flex w-full items-center justify-center gap-2 rounded-2xl bg-[#15233f] px-6 py-4 font-black text-white shadow-lg"><RotateCcw size={18}/> {outOfLives ? "Start Fresh Adventure" : "Play Again"}</button></div>
    </div>
  </div>;
}
