...
export default function ParentLearnerSwitcher() {
  ...
  useEffect(() => {
    let active = true;
    getMyLearners()
      .then((items) => { if (active) setLearners(items); })
      .catch(() => { if (active) setLearners([]); })
  ...
  return (
    <label className="inline-flex items-center gap-2 rounded-2xl border border-violet-100 bg-violet-50/80 px-3 py-2 text-xs font-black text-violet-900 shadow-sm">
      <span className="hidden sm:inline">Viewing</span>
      <select
        aria-label="Select learner"
        value={selected?.learner_id ?? ""}
        onChange={(e) => selectLearner(e.target.value)}
        className="max-w-40 bg-transparent font-black outline-none"
      >
        {learners.map((learner) => (
          <option key={learner.learner_id} value={learner.learner_id}>
            {learner.avatar_emoji || "👤"} {learner.display_name} · {learner.grade || "Learner"}
          </option>
        ))}
      </select>
