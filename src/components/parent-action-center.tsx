...
export default function ParentActionCenter({ studentId }: { studentId: string }) {
  ...
  return <section className="mt-5 rounded-[2rem] bg-white p-6 shadow-sm ring-1 ring-slate-100 sm:p-8">
    <div className="flex items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.16em] text-[#087b92]">Parent action center</p><h3 className="mt-1 text-2xl font-black text-[#071b3a]">Turn learning signals into action</h3><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">Choose a recommended follow-up, track what you are doing, and mark it complete when the learner has received support.</p></div><CircleAlert className="hidden text-[#0a9ca6] sm:block" /></div>

    {loading ? <div className="mt-5 rounded-2xl bg-slate-50 p-5 text-sm font-bold text-slate-500">Loading actions…</div> : <>
      {recommendations.filter(r => !existingRecommendationKeys.has(r.recommendation_id)).length > 0 && <div className="mt-6"><p className="text-sm font-black text-slate-800">Recommended follow-up</p><div className="mt-3 grid gap-3 lg:grid-cols-2">{recommendations.filter(r => !existingRecommendationKeys.has(r.recommendation_id)).map(r => <div key={r.recommendation_id} className="rounded-2xl border border-[#c9ebeb] bg-[#effaf9] p-4"><div className="flex gap-3"><div className="flex-1"><p className="font-black text-slate-800">{r.title}</p><p className="mt-1 text-sm leading-6 text-slate-600">{r.description}</p><p className="mt-2 text-xs font-semibold text-[#087b92]">{r.reason}</p></div><button onClick={() => void createRecommendation(r)} disabled={working === r.recommendation_id} className="h-fit rounded-xl bg-[#087b92] px-3 py-2 text-xs font-black text-white hover:bg-[#076779] disabled:opacity-50">{working === r.recommendation_id ? "Adding…" : "Add action"}</button></div></div>)}</div></div>}

      {open.length > 0 && <div className="mt-6"><p className="text-sm font-black text-slate-800">Open follow-ups <span className="ml-1 rounded-full bg-amber-100 px-2 py-0.5 text-xs text-amber-700">{open.length}</span></p><div className="mt-3 space-y-3">{open.map(a => <div key={a.action_id} className="rounded-2xl border border-amber-100 bg-amber-50/50 p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div className="flex gap-3"><Clock3 className="mt-0.5 shrink-0 text-amber-600" size={18} /><div><p className="font-black text-slate-800">{a.title}</p><p className="mt-1 text-sm leading-6 text-slate-600">{a.description}</p>{a.due_at && <p className="mt-2 text-xs font-bold text-amber-700">Due {new Date(a.due_at).toLocaleDateString()}</p>}</div></div><div className="flex gap-2"><button onClick={() => void setStatus(a.action_id, "in_progress")} disabled={working === a.action_id} className="rounded-xl bg-white px-3 py-2 text-xs font-black text-[#087b92] ring-1 ring-violet-100 hover:bg-[#e7f6f9]"><PlayCircle size={14} className="mr-1 inline" />Start</button><button onClick={() => void setStatus(a.action_id, "completed")} disabled={working === a.action_id} className="rounded-xl bg-emerald-600 px-3 py-2 text-xs font-black text-white hover:bg-emerald-700"><CheckCircle2 size={14} className="mr-1 inline" />Complete</button><button onClick={() => void setStatus(a.action_id, "dismissed")} disabled={working === a.action_id} className="rounded-xl bg-white px-3 py-2 text-xs font-black text-slate-500 ring-1 ring-slate-200"><XCircle size={14} className="mr-1 inline" />Dismiss</button></div></div></div>)}</div><div className="mt-3"><input value={note} onChange={e => setNote(e.target.value)} placeholder="Optional completion note" className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none focus:border-[#0a9ca6]" /></div></div>}
