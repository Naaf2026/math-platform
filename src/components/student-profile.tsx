...
export function StudentProfilePill() {
  ...
  if (!stats) {
    return <div className="h-12 w-12 animate-pulse rounded-2xl bg-slate-100" aria-hidden="true" />;
  }
  ...
  return (
    <Link href="/profile" className="group hidden lg:flex w-16 flex-col items-center gap-1 rounded-2xl p-1.5 transition hover:bg-violet-50" title="Open student profile" aria-label="Open student profile">
      <div className="relative flex h-11 w-11 items-center justify-center rounded-2xl bg-gradient-to-br from-violet-600 via-indigo-500 to-cyan-400 text-base font-black text-white shadow-lg shadow-violet-200 ring-2 ring-white">
        {initial}
        <span className="absolute -right-2 -top-2 flex h-6 min-w-6 items-center justify-center rounded-full border-2 border-white bg-yellow-400 px-1 text-[10px] font-black text-slate-900 shadow-sm">
          {level}
        </span>
      </div>
      <div className="h-1.5 w-12 overflow-hidden rounded-full bg-slate-200">
        <div className="h-full rounded-full bg-gradient-to-r from-violet-500 to-cyan-400 transition-all" style={{ width: `${percent}%` }} />
      </div>
    </Link>
  );
}
    ...
    <section className={compact ? "rounded-3xl border border-violet-100 bg-white p-5 shadow-sm" : "overflow-hidden rounded-[2rem] border border-violet-100 bg-white shadow-xl shadow-violet-100/50"}>
      <div className="bg-gradient-to-br from-violet-600 via-indigo-600 to-cyan-500 p-6 text-white">
        <div className="flex items-center gap-4">
          <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-3xl bg-white/20 text-2xl font-black ring-4 ring-white/20">
            {initial}
          </div>
