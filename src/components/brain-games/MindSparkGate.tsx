...
export default function MindSparkGate({ gameKey, gameTitle, onStarted, children, className }: Props) {
  ...
  const prepare = async () => {
    ...
    try {
      const { data, error: rpcError } = await createClient().rpc("get_mind_spark_status");
      ...
      setOpen(true);
    } catch (e: any) { setError(e?.message || "Unable to load Mind Sparks."); }
  ...
  return <>
    ...
    {open && status && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/55 p-4 backdrop-blur-sm" role="dialog" aria-modal="true">
