...
export default function MindSparkGate({ gameKey, gameTitle, onStarted, children, className }: Props) {
  ...
  const prepare = async () => {
    ...
    try {
      ...
      setStatus({
        ...
        free_play_available: Boolean(row.free_play_available),
        free_play_game_key: row.free_play_game_key ?? null,
      });
      setOpen(true);
    } catch (e: any) {
  ...
  return (
    <>
      <button type="button" onClick={prepare} disabled={loading} className={className}>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/55 p-4 backdrop-blur-sm" role="dialog" aria-modal="true">
          <div className="w-full max-w-md overflow-hidden rounded-[30px] border-4 border-white bg-white shadow-2xl">
            <div className="bg-gradient-to-br from-cyan-500 via-blue-600 to-purple-600 p-6 text-center text-white">
              ...
              <h2 className="mt-3 text-2xl font-black">Ready to play?</h2>
              <p className="mt-1 font-bold text-white/85">{gameTitle}</p>
            </div>
            <div className="p-6 text-center">
              ...
              <div className="mt-5 flex gap-3">
                <button type="button" onClick={() => setOpen(false)} className="flex-1 rounded-2xl bg-slate-100 px-4 py-3 font-black text-slate-700">Cancel</button>
                <button type="button" onClick={start} disabled={loading || insufficient} className="flex-1 rounded-2xl bg-gradient-to-r from-cyan-500 to-blue-600 px-4 py-3 font-black text-white disabled:cursor-not-allowed disabled:opacity-50"><Zap className="mr-1 inline" size={17}/> {loading ? "Starting…" : "Play Now"}</button>
              </div>
            </div>
          </div>
        </div>
